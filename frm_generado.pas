...
uses
  ...
  PanelDown, Newpanel, utilerias, DateUtils, UnitExcel, ComObj,
  jpeg, ZAbstractRODataset, ZDataset, RxLookup, rxToolEdit, rxCurrEdit,
...
//continuainserccion de datos
  try
    ...
    begin
      rInforme.LoadFromFile(global_files + 'ReportadoVsGenerado.fr3');
      ...
      rInforme.ShowReport;
      if not FileExists(global_files + 'ReportadoVsGenerado.fr3') then
        showmessage('El archivo de reporte ReportadoVsGenerado.fr3 no existe, notifique al administrador del sistema');
    end
    else
...
begin
  tdpendiente.Color := global_color_entrada
end;

procedure TfrmGenerado.tdPendienteExit(Sender: TObject);
begin
  tdpendiente.Color := global_color_salida
end;
...
begin
  tdtotalbitacora.Color := global_color_entrada
end;

procedure TfrmGenerado.tdTotalBitacoraExit(Sender: TObject);
begin
  tdtotalbitacora.Color := global_color_salida
end;
...
begin
  tdtotalgenerado.Color := global_color_entrada
end;

procedure TfrmGenerado.tdTotalGeneradoExit(Sender: TObject);
begin
  tdtotalgenerado.Color := global_color_salida
end;
