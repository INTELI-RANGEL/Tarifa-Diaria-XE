...
uses
  ...
  StdCtrls, ComCtrls, ZAbstractDataset, RxMemDS, Utilerias,
  ExtCtrls, Newpanel, udbgrid, UnitExcepciones, DBDateTimePicker, Buttons,
  RXDBCtrl, JvExDBGrids, JvDBGrid, JvDBUltimGrid, JvDBGridFooter;
...
type
  TfrmProrrateoPernocta = class( TForm )
    ...
    movimientosxfolios: TZQuery;
    dsmovimientosxfolios: TDataSource;
    movimientosxfoliossContrato: TStringField;
    movimientosxfoliosdIdFecha: TDateField;
    movimientosxfoliosiIdDiario: TIntegerField;
    movimientosxfoliossNumeroOrden: TStringField;
    movimientosxfoliossFolio: TStringField;
    movimientosxfoliossFactor: TFloatField;
    movimientosxfoliosdCantHH: TFloatField;
    movimientosxfoliossFactorBarco: TFloatField;
    movimientosxfolioslFactor: TStringField;
    Panel1: TPanel;
    Label4: TLabel;
    lblTotal: TLabel;
    ...
    procedure movimientosxfoliossFactorValidate(Sender: TField);
  private
...
procedure TfrmProrrateoPernocta.qryMovimientosBarcosAfterScroll(
  DataSet: TDataSet);
VAR
  QrTotal:TzReadOnlyQuery;
begin

    movimientosxfolios.Active:=false;
    ...
    begin
       ...
       if Not InProcess then
       begin
          movimientosxfolios.ParamByname('Contrato').AsString:=QryMovimientosBarcos.FieldByName('sContrato').AsString;
          movimientosxfolios.ParamByname('Fecha').AsDate:=QryMovimientosBarcos.FieldByName('dIdFecha').AsDateTime;
          movimientosxfolios.ParamByname('Diario').AsInteger:=QryMovimientosBarcos.FieldByName('iIdDiario').AsInteger;
          movimientosxfolios.Open;

          if movimientosxfolios.RecordCount>0 then
          begin
            QrTotal:=TzReadOnlyQuery.Create(nil);
            try
              QrTotal.Connection:=Connection.zConnection;
              QrTotal.SQL.Text:='select ifnull(sum(sFactor),0) as suma from movimientosxfolios where sContrato=:Contrato '+
              'and dIdFecha=:fecha and iIdDiario=:Diario ';
              QrTotal.ParamByName('Contrato').AsString:=movimientosxfolios.FieldByName('sContrato').AsString;
              QrTotal.ParamByName('Fecha').Asdate:=movimientosxfolios.FieldByName('dIdFecha').AsDatetime;
              QrTotal.ParamByName('Diario').AsInteger:=movimientosxfolios.FieldByName('iIdDiario').AsInteger;
             // QrTotal.ParamByName('Folio').AsString:=movimientosxfolios.FieldByName('sFolio').AsString;
              QrTotal.Open;
              if QrTotal.RecordCount=1 then
                lblTotal.Caption:= QrTotal.FieldByName('suma').AsString;
            finally
              QrTotal.Destroy
            end;
          end;
  ...
  try
      ...
      begin
          ...
          begin
              ...
              TdProrrateoFolio(QryMovimientosBarcos.FieldByName('sContrato').AsString,QryMovimientosBarcos.FieldByName('dIdFecha').AsDateTime,QryMovimientosBarcos.FieldByName('iIdDiario').AsInteger);
...
begin
    ...
    begin
       ...
       Begin
           dCantidad := connection.QryBusca.FieldValues[ 'dCantidad' ];
           qryMovimientosBarcos.disableControls;
           qryMDOrdenes.DisableControls;
        try
           qryMovimientosBarcos.First;
           while not qryMovimientosBarcos.Eof  do
           begin
          ...
          qryMovimientosBarcos.First;
          qryMovimientosBarcos.enableControls;
          qryMDOrdenes.First;
          qryMDOrdenes.EnableControls;
        end;
       End
       else
          dCantidad := 0;
    ...
    InProcess:=false;
    qryMovimientosBarcosAfterScroll(qryMovimientosBarcos);

end;

procedure TfrmProrrateoPernocta.movimientosxfoliossFactorValidate(
  Sender: TField);
var
  QrTotal:TzReadOnlyQuery;
begin
  ...
  try
    QrTotal.Connection:=Connection.zConnection;
    QrTotal.SQL.Text:='select ifnull(sum(sFactor),0) as suma from movimientosxfolios where sContrato=:Contrato '+
    'and dIdFecha=:fecha and iIdDiario=:Diario and sFolio<>:Folio';
    QrTotal.ParamByName('Contrato').AsString:=movimientosxfolios.FieldByName('sContrato').AsString;
    QrTotal.ParamByName('Fecha').Asdate:=movimientosxfolios.FieldByName('dIdFecha').AsDatetime;
    QrTotal.ParamByName('Diario').AsInteger:=movimientosxfolios.FieldByName('iIdDiario').AsInteger;
    QrTotal.ParamByName('Folio').AsString:=movimientosxfolios.FieldByName('sFolio').AsString;
    QrTotal.Open;
    if QrTotal.RecordCount=1 then
      if (QrTotal.FieldByName('suma').AsFloat+sender.NewValue)>
      QryMovimientosBarcos.FieldByName('sFactor').AsFloat then
        raise Exception.Create('No Puede Capturar mas de lo Generado en el dia. ')
      else
        lblTotal.Caption:= FloatTostr(QrTotal.FieldByName('suma').AsFloat+sender.NewValue);
