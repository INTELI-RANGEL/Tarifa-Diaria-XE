unit frm_bitacoradepartamental_Tierra;
...
uses
  ...
  cxButtons, cxTextEdit, cxGroupBox, cxDBEdit, cxMaskEdit, cxDropDownEdit,
  cxStyles, dxSkinscxPCPainter, cxCustomData, cxFilter, cxData, cxDataStorage,
  cxNavigator, cxDBData, cxGridLevel, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxClasses, cxGridCustomView, cxGrid, AdvGlassButton,
  cxSpinEdit, cxPC, dxDockControl, dxDockPanel, cxLookupEdit, cxDBLookupEdit,
  cxDBLookupComboBox;
type
Evalidaciones = class(Exception)
end;
type
  TfrmBitacoraDepartamental_Tierra = class(TForm)
    {$REGION 'Componentes'}
    ...
    QrFrentesFechaInicio: TDateField;
    QrFrentesFechaFinal: TDateField;
    ...
    lblCortes: TLabel;
    LCmbCortes: TcxLookupComboBox;
    ItmRecursos: TMenuItem;
    {$ENDREGION}
   ...
   // Procedure GeneraReporteAvances_PDF(RTipo:FtTipo; RImpresion:FtActividades);
    ...
    procedure chkEstatusHoraEnter(Sender: TObject);
    ...
    procedure VisualizarGerencial1Click(Sender: TObject);
    ...
    procedure LCmbCortesPropertiesCloseUp(Sender: TObject);
  private
...
var
  frmBitacoraDepartamental_Tierra: TfrmBitacoraDepartamental_Tierra;
  sDescripcion: string;
  ...
  dCantidadOrden,solicitadoE,solicitadop: Double;
  dError: Currency;
  txtMensaje: string;
...
uses frm_comentariosxanexo, UnitExcepciones, frm_OpcionesGerencial, frm_NotasGenerales, frm_ReporteDiarioTurno,
  frm_ReprogramacionFolio, UFrmRecursosTierra;
...
procedure TfrmBitacoraDepartamental_Tierra.FiltrarFolios(Sender : TObject);
begin
  LblTodos.Font.Style :=[];
  ...
  QrFrentes.SQL.Clear;
  QrFrentes.SQL.Add('SELECT b.sidtipomovimiento,ot.sContrato, ot.sNumeroOrden, ot.cIdStatus, '+
                    'IF(b.sContrato = ot.sContrato,"Si","No") as Orden, c.sIdConvenio as Convenio, c.dFechaInicio as FechaInicio, c.dFechaFinal as FechaFinal '+
                    'FROM ordenesdetrabajo AS ot  '+
                    'left join convenios c on (c.sContrato = ot.sContrato and ot.sNumeroOrden = c.sNumeroOrden and (:Fecha between c.dFechaInicio and c.dFechaFinal)) ');
...
function TfrmBitacoraDepartamental_Tierra.ValidarConfJor;
var
...
function TfrmBitacoraDepartamental_Tierra.ValidaBarco(dParamPersonal: string): boolean;
var
...
procedure TfrmBitacoraDepartamental_Tierra.Vigencias();
begin
...
procedure TfrmBitacoraDepartamental_Tierra.VisualizarGerencial1Click(
  Sender: TObject);
begin
...
function TfrmBitacoraDepartamental_Tierra.fnActualizaAcumuladosContrato(sParamOpcion, sParamNumeroActividad: string;
  dParamCantidadInstalar, dParamInstalado, dParamExcedente, dParamCantidad: double): Boolean;
begin
  try
...
function TfrmBitacoraDepartamental_Tierra.GetItemByName(Wnd : hWnd;  hItem : HTREEITEM;szItemName : LPCTSTR) : HTREEITEM ;
var
...
function TfrmBitacoraDepartamental_Tierra.fnActualizaAcumuladosOrden(sParamOpcion, sParamWbs, sParamNumeroActividad: string;
  dParamCantidadInstalar, dParamInstalado, dParamExcedente, dParamCantidad: double): Boolean;
begin
  try
    connection.zCommand.Active := False;
...
function TfrmBitacoraDepartamental_Tierra.fnValidaPartidaAnexo(sParamNumeroActividad: string): boolean;
begin
  dExcedenteAnexo := 0;
  dInstaladoAnexo := 0;
  dCantidadAnexo := 0;
...
function TfrmBitacoraDepartamental_Tierra.fnValidaPartidaOrden(sParamWbs, sParamNumeroActividad: string): boolean;
begin
  dExcedenteOrden := 0;
  dInstaladoOrden := 0;
  dCantidadOrden  := 0;
...
function TfrmBitacoraDepartamental_Tierra.fnValidaPartidaOrdenPorcentaje(sParamWbs, sParamNumeroActividad: string): boolean;
begin
  dExcedenteOrden := 0;
  dInstaladoOrden := 0;
  dCantidadOrden := 0;
...
procedure TfrmBitacoraDepartamental_Tierra.FormShow(Sender: TObject);
var
...
procedure TfrmBitacoraDepartamental_Tierra.tdIdFechaExit(Sender: TObject);
begin
  lBorra := False;
  if Param_Frente <> '' then
  begin
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnAddClick(Sender: TObject);
var
   sHora : string;
begin
  try
      ...
      if (sHora > '05:00') and (sHora <= '17:00')  then
      begin
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnEditClick(Sender: TObject);
var
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnPostClick(Sender: TObject);
var
    lFiltro,
    lEfectivo,
    Consolidado,
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnCancelClick(Sender: TObject);
begin
  tsNumeroActividad.ReadOnly  := True;
  tsIdtipoMovimiento.ReadOnly := True;
  tdCantidad.ReadOnly         := True;
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnRefreshClick(Sender: TObject);
begin
  QryBitacora.Active := False;
  QryBitacora.Open;
...
procedure TfrmBitacoraDepartamental_Tierra.GridNotasCellClick(Column: TColumn);
begin
    btnCancelN.Click;
    lMostrarNotas := True;
...
procedure TfrmBitacoraDepartamental_Tierra.Grid_BitacoraCellClick(Column: TColumn);
begin
    lMostrarNotas := False;
    //Colores Descripcion
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnDeleteClick(Sender: TObject);
var
  lBorra: Boolean;
begin
  try
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnExitClick(Sender: TObject);
begin
  global_Editor := '';
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroOrdenKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    Grid_Bitacora.SetFocus
...
procedure TfrmBitacoraDepartamental_Tierra.tsPasswordEnter(Sender: TObject);
begin
    tsPassword.Color := global_color_entrada;
end;

procedure TfrmBitacoraDepartamental_Tierra.tsPasswordExit(Sender: TObject);
begin
    tsPassword.Color := global_color_salida;
end;

procedure TfrmBitacoraDepartamental_Tierra.tssIdClasificacionEnter(Sender: TObject);
begin
    tssIdClasificacion.Color := global_color_entrada;
end;

procedure TfrmBitacoraDepartamental_Tierra.tssIdClasificacionExit(Sender: TObject);
begin
    tssIdClasificacion.Color := global_color_salida;
  if QrClasificacion.FieldByName('sClasificacion').AsString='Nota de Horario' then
  begin
    LCmbCortes.Visible:=True;
    lblCortes.Visible:=True;
    LCmbCortes.Enabled:=True;
    LCmbCortes.SetFocus;
  end
...
procedure TfrmBitacoraDepartamental_Tierra.tssIdClasificacionKeyPress(
...
procedure TfrmBitacoraDepartamental_Tierra.TvFrentesChange(Sender: TObject;
  Node: TTreeNode);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.TvFrentesCollapsing(Sender: TObject;
  Node: TTreeNode; var AllowCollapse: Boolean);
begin
  AllowCollapse:=False;
end;

procedure TfrmBitacoraDepartamental_Tierra.TvFrentesCustomDrawItem(
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividadKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
      ...
      if QryBitacora.FieldByName('sTipoAnexo').AsString  = 'PU' then
        if tdCantidad.Visible = True then
           tdCantidad.SetFocus;
  end;
end;

procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividadMouseMove(
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividad_ADMEnter(
  Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividad_ADMExit(
  Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividad_ADMKeyPress(
  Sender: TObject; var Key: Char);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividad_PUEnter(
  Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividad_PUExit(Sender: TObject);
begin
    tdPonderado.Value  := 0;
    tdCantidad.Value   := 0;
    tmDescripcion.Text := '';
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividad_PUKeyPress(
  Sender: TObject; var Key: Char);
begin
    if key=#13 then
       tsNumeroActividad_ADM.SetFocus;
...
procedure TfrmBitacoraDepartamental_Tierra.tsIdTipoMovimientoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    tdCantidad.SetFocus
...
procedure TfrmBitacoraDepartamental_Tierra.tdAvanceChange(Sender: TObject);
begin
     //activapop(frmBitacoraDepartamental_2, popupprincipal);
end;

procedure TfrmBitacoraDepartamental_Tierra.tdAvanceEnter(Sender: TObject);
begin
    tdAvance.Color := global_color_entrada;
end;

procedure TfrmBitacoraDepartamental_Tierra.tdAvanceExit(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tdAvanceGlobalChange(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tdAvanceKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    tsHoraInicio.SetFocus
end;

function TfrmBitacoraDepartamental_Tierra.lExisteActividadAnexo(sActividad: string): Boolean;
begin
  if TiposdeMovimiento.FieldValues['sClasificacion'] = 'Notas' then
  begin
    sDescripcion := '';
...
procedure TfrmBitacoraDepartamental_Tierra.ListaObjetoDblClick(Sender: TObject);
begin
    GridMaterialesxPartida.SetFocus;
end;

procedure TfrmBitacoraDepartamental_Tierra.ListaObjetoExit(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.ListaObjetoKeyPress(Sender: TObject;
  var Key: Char);
begin
    if Key = #13 then
       GridMaterialesxPartida.SetFocus;
...
procedure TfrmBitacoraDepartamental_Tierra.FormClose(Sender: TObject;
...
procedure TfrmBitacoraDepartamental_Tierra.Insertar1Click(Sender: TObject);
begin
  frmBarra1.btnAdd.Click
end;

procedure TfrmBitacoraDepartamental_Tierra.Editar1Click(Sender: TObject);
begin
  frmBarra1.btnEdit.Click
end;

procedure TfrmBitacoraDepartamental_Tierra.popGerencialClick(Sender: TObject);
begin
     Application.CreateForm(TfrmOpcionesGerencial, frmOpcionesGerencial);
     frmOpcionesGerencial.showModal
end;

procedure TfrmBitacoraDepartamental_Tierra.Registrar1Click(Sender: TObject);
begin
  frmBarra1.btnPost.Click
end;

procedure TfrmBitacoraDepartamental_Tierra.RevisarOrtografia2Click(Sender: TObject);
var
  WindowName: string;
...
procedure TfrmBitacoraDepartamental_Tierra.bitacoradematerialesAfterEdit(
  DataSet: TDataSet);
begin
  if lBorra = True then
  begin
...
procedure TfrmBitacoraDepartamental_Tierra.bitacoradematerialesAfterInsert(
  DataSet: TDataSet);
begin
   if lBorra = True then
    if QryBitacora.RecordCount > 0 then
...
procedure TfrmBitacoraDepartamental_Tierra.bitacoradematerialesBeforeDelete(
  DataSet: TDataSet);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.bitacoradematerialesBeforeInsert(
  DataSet: TDataSet);
begin
    if qryNotasGerencial.RecordCount = 0 then
    begin
...
procedure TfrmBitacoraDepartamental_Tierra.bitacoradematerialesBeforePost(
  DataSet: TDataSet);
var
 ...
 try
    ...
    if (bitacorademateriales.FieldValues['sIdMaterial'] = null) then
      ...
      begin
          ...
          if  dCantidadSalidas < (dCantidadSuma + bitacorademateriales.FieldByName('dCantidad').AsFloat) then
          begin
...
procedure TfrmBitacoraDepartamental_Tierra.bitacoradematerialesCalcFields(
  DataSet: TDataSet);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.bitacoradematerialessIdMaterialChange(
  Sender: TField);
begin
 ...
 //aqui va para cuando cambia por partida
  ...
  begin
    ...
    Connection.qryBusca.SQL.Add('select SubStr(aa.mDescripcion, 1, 255) as sDescripcion, aa.sMedida, aa.sAnexo from actividadesxanexo aa '+
              'inner join anexos a on (aa.sAnexo = a.sTipo and a.sTipo = "MATERIAL" or a.sTipo = "ANEXO") '+
...
procedure TfrmBitacoraDepartamental_Tierra.btn1Click(Sender: TObject);
begin
  FiltrarFolios(LblTodos);
end;

procedure TfrmBitacoraDepartamental_Tierra.btnAddNClick(Sender: TObject);
begin
    if QryBitacora.RecordCount > 0 then
    begin
        frmBarra1.btnCancelClick(Sender);
        ...
        cxCerrado.Visible := True;
        cxAbierto.Visible := False;
...
procedure TfrmBitacoraDepartamental_Tierra.btnCancelNClick(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.btnDeleteNClick(Sender: TObject);
var
  SavePlace   : TBookmark;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.btnDeleteDClick(Sender: TObject);
var
  SavePlace   : TBookmark;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.btnDownClick(Sender: TObject);
begin
    OrdenarNotas('Abajo');
end;

procedure TfrmBitacoraDepartamental_Tierra.btnEditNClick(Sender: TObject);
begin
   if QryBitacora.RecordCount > 0 then
    begin
        frmBarra1.btnCancelClick(Sender);
...
procedure TfrmBitacoraDepartamental_Tierra.btnMayusClick(Sender: TObject);
begin
  tmDescripcion.Text := UpperCase(tmDescripcion.Text);
end;

procedure TfrmBitacoraDepartamental_Tierra.btnNotasClick(Sender: TObject);
begin
  frm_NotasGenerales.FechaReporte := tdIdFecha.Date;
  Application.CreateForm(TfrmNotasGenerales, frmNotasGenerales);
  frmNotasGenerales.ShowModal;
end;

procedure TfrmBitacoraDepartamental_Tierra.btnPostNClick(Sender: TObject);
var
   iDiario, iRegistro     : integer;
   lIncorrecto : boolean;
...
begin
    ...
    if LCmbCortes.Visible then
      if LCmbCortes.EditText='' then
      begin
        messageDLG('Debe elegir una nota principal', mtInformation, [mbOk], 0);
        LCmbCortes.SetFocus;
        exit;
   ...
   {Continua insercion de datos}
    ...
    if qryNotasGerencial.RecordCount > 0 then
        if dCantidadOld = tdCantidad.Value then
           bContinue := False;
                ...
                PartidaExistente(sNumeroActividad, sWbsFormulario, sTiempoEfectivo);
                   begin
                      ...
                      connection.zCommand.SQL.Add('INSERT INTO bitacoradeactividades ( sContrato, sIdConvenio, dIdFecha , iIdDiario, sIdTurno, sNumeroActividad, ' +
                        ' sNumeroOrden, sWbs, sIdTipoMovimiento, sHoraInicio, sHoraFinal, mDescripcion, lImprime, sConceptoGerencial, iIdDiarioNota, sIdClasificacion, iIdTarea, eTipoActividad, dCantidad, dAvance,iHermano) ' +
                        ' VALUES (:contrato, :Convenio, :fecha, :diario, :turno, :actividad, :orden, :Wbs, :tipo, :inicio, :final, :descripcion, :imprime, :Concepto, :Id, :clasificacion, :diario, :tipoActividad, :cantidad, :avance,:Hermano)');
                      Connection.zCommand.Params.ParamByName('contrato').AsString      := param_Global_Contrato;
                      connection.zCommand.Params.ParamByName('Convenio').AsString      := QrFrentes.FieldByName('Convenio').AsString;
                      Connection.zCommand.Params.ParamByName('fecha').AsDate           := tdIdFecha.Date;
                      Connection.zCommand.Params.ParamByName('diario').AsInteger       := iDiario;
                      ...
                      if QrClasificacion.FieldByName('sClasificacion').AsString='Nota de Horario' then
                      begin
                        Connection.zCommand.Params.ParamByName('tipo').AsString          := 'EN';
                        Connection.zCommand.Params.ParamByName('Hermano').AsInteger:= LCmbCortes.EditValue;

                      end
                      else
                      begin
                        Connection.zCommand.Params.ParamByName('tipo').AsString          := 'ED';
                        Connection.zCommand.Params.ParamByName('Hermano').AsInteger:=-1;
                      end;
                      Connection.zCommand.Params.ParamByName('inicio').AsString        := tsHoraInicio.Text;
                      Connection.zCommand.Params.ParamByName('final').AsString         := tsHoraFinal.Text;
                      ...
                      begin
                        connection.zCommand.Active:=False;
                        connection.zCommand.SQL.Text:='update bitacoradeactividades set iHermano=iIdActividad where iIdActividad=:Actividad';
                        connection.zCommand.ParamByName('actividad').AsInteger:=LCmbCortes.EditValue;
                        connection.zCommand.ExecSQL;
                      end;
    ...
    begin
        {$REGION 'Actuliza Notas'}
          try
              ...
              QryUpdate.Active := False;
              QryUpdate.SQL.Clear;
              QryUpdate.SQL.Add('UPDATE bitacoradeactividades SET mDescripcion = :descripcion, sHoraInicio = :HoraInicio, sHoraFinal = :HoraFinal ' +
                                'where sContrato = :contrato and sIdConvenio =:Convenio And dIdFecha = :fecha And iIdDiario = :diario  ');
              QryUpdate.Params.ParamByName('contrato').AsString    := param_Global_Contrato;
              QryUpdate.Params.ParamByName('Convenio').AsString    := QrFrentes.FieldByName('Convenio').AsString;
              QryUpdate.Params.ParamByName('fecha').AsDate         := tdIdFecha.Date;
...
procedure TfrmBitacoraDepartamental_Tierra.btnUpClick(Sender: TObject);
begin
    OrdenarNotas('Arriba');
end;

procedure TfrmBitacoraDepartamental_Tierra.BView_ActividadesDblClick(
  Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.Can1Click(Sender: TObject);
begin
  frmBarra1.btnCancel.Click
end;

procedure TfrmBitacoraDepartamental_Tierra.Eliminar1Click(Sender: TObject);
begin
  frmBarra1.btnDelete.Click
end;

procedure TfrmBitacoraDepartamental_Tierra.Refresh1Click(Sender: TObject);
begin
  frmBarra1.btnRefresh.Click
end;

procedure TfrmBitacoraDepartamental_Tierra.Salir1Click(Sender: TObject);
begin
  frmBarra1.btnExit.Click
end;

procedure TfrmBitacoraDepartamental_Tierra.sNumeroActividadStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
begin
    AStyle:=cxNormal;
...
procedure TfrmBitacoraDepartamental_Tierra.SpeedButton1Click(Sender: TObject);
begin
  FiltrarFolios(LblReportados);
end;

procedure TfrmBitacoraDepartamental_Tierra.LblReportadosClick(Sender: TObject);
begin
  FiltrarFolios(Sender);
end;

procedure TfrmBitacoraDepartamental_Tierra.LblTodosClick(Sender: TObject);
begin
 FiltrarFolios(Sender);
end;

procedure TfrmBitacoraDepartamental_Tierra.LCmbCortesPropertiesCloseUp(
  Sender: TObject);
begin
  tsHoraInicio.Text:=QrNotasCortes.FieldByName('sHoraInicio').AsString;
  tsHoraFinal.Text:=QrNotasCortes.FieldByName('sHoraFinal').AsString;
end;

procedure TfrmBitacoraDepartamental_Tierra.LeerDatasets(PsContrato: string; PsNumeroOrden: string);
var
  ListItem: TListItem;
  qryGrupos: tZReadOnlyQuery;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tdCantidadKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tdEditaAvanceEnter(Sender: TObject);
begin
    tdEditaAvance.Color := global_color_entrada;
end;

procedure TfrmBitacoraDepartamental_Tierra.tdEditaAvanceExit(Sender: TObject);
begin
    tdEditaAvance.Color := clActiveCaption;
end;

procedure TfrmBitacoraDepartamental_Tierra.tdEditaAvanceKeyPress(Sender: TObject;
  var Key: Char);
begin
    if key=#13 then
       cxedita.SetFocus;
...
procedure TfrmBitacoraDepartamental_Tierra.grid_bitacoraEnter(Sender: TObject);
var
  iCheck: Byte;
...
procedure TfrmBitacoraDepartamental_Tierra.tdIdFechaEnter(Sender: TObject);
begin
  frmBarra1.btnCancel.Click;
  tdIdFecha.Color := global_color_entrada;
end;

procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividadEnter(
  Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividadExit(Sender: TObject);
begin
   tdPonderado.Value  := 0;
   tdCantidad.Value   := 0;
   tmDescripcion.Text := '';
...
procedure TfrmBitacoraDepartamental_Tierra.tsHoraFinalChange(Sender: TObject);
begin
   //activapop(frmBitacoraDepartamental_2, popupprincipal);
end;

procedure TfrmBitacoraDepartamental_Tierra.tsHoraFinalEnter(Sender: TObject);
begin
    tsHoraFinal.Color := global_color_entrada;   
end;

procedure TfrmBitacoraDepartamental_Tierra.tsHoraFinalExit(Sender: TObject);
begin
    tsHoraFinal.Color := global_color_salida;
end;

procedure TfrmBitacoraDepartamental_Tierra.tsHoraFinalKeyPress(Sender: TObject;
  var Key: Char);
begin
    if (sOpcion = 'Nuevo') or (sOpcion = 'Edit') then
    begin
        if Key = #13 then
...
procedure TfrmBitacoraDepartamental_Tierra.tsHoraInicioChange(Sender: TObject);
begin
    //activapop(frmBitacoraDepartamental_2, popupprincipal);
end;

procedure TfrmBitacoraDepartamental_Tierra.tsHoraInicioEnter(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tsHoraInicioExit(Sender: TObject);
begin
    tsHoraInicio.Color := global_color_salida;
end;

procedure TfrmBitacoraDepartamental_Tierra.tsHoraInicioKeyPress(Sender: TObject;
  var Key: Char);
begin
    if Key = #13 then
       tsHoraFinal.SetFocus;
...
procedure TfrmBitacoraDepartamental_Tierra.tsIdTipoMovimientoEnter(
  Sender: TObject);
begin
  tsIdTipoMovimiento.Color := global_color_entrada
end;

procedure TfrmBitacoraDepartamental_Tierra.tsIdTipoMovimientoExit(
  Sender: TObject);
begin
  tsIdTipoMovimiento.Color := global_color_salida;
end;

procedure TfrmBitacoraDepartamental_Tierra.tdCantidadChange(Sender: TObject);
begin
  TRxCalcEditChangef(tdCantidad, 'Cantidad');
end;

procedure TfrmBitacoraDepartamental_Tierra.tdCantidadEnter(Sender: TObject);
begin
    tdCantidad.Color := global_color_entrada;
end;

procedure TfrmBitacoraDepartamental_Tierra.tdCantidadExit(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tmDescripcionDblClick(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tmDescripcion_GerencialEnter(
  Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.popImprimeGerencialClick(Sender: TObject);
begin
    if qryNotasGerencial.RecordCount > 0 then
    begin
        connection.zCommand.Active := False;
...
procedure TfrmBitacoraDepartamental_Tierra.popNotasPopup(Sender: TObject);
begin
  ItmRecursos.Visible:=False;
  if connection.contrato.FieldByName('eLugarOt').AsString='Tierra' then
  begin
    ItmRecursos.Visible:=true;
    if qryNotasGerencial.RecordCount=0 then
      ItmRecursos.Enabled:=False
    else
      ItmRecursos.Enabled:=True;
  end;


end;

procedure TfrmBitacoraDepartamental_Tierra.grid_igualesGetCellParams(
...
procedure TfrmBitacoraDepartamental_Tierra.grid_igualesTitleClick(Column: TColumn);
begin
  UtGrid2.DbGridTitleClick(Column);
end;

procedure TfrmBitacoraDepartamental_Tierra.hGerencialStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
begin
    AStyle := cxNormal;
...
procedure TfrmBitacoraDepartamental_Tierra.Grid_BitacoraGetCellParams(
  Sender: TObject; Field: TField; AFont: TFont; var Background: TColor;
  Highlight: Boolean);
var x: integer;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.Grid_BitacoraTitleClick(Column: TColumn);
begin
  UtGrid.DbGridTitleClick(Column);
end;

procedure TfrmBitacoraDepartamental_Tierra.TiposdeMovimientoAfterScroll(
  DataSet: TDataSet);
begin
  if TiposdeMovimiento.FieldValues['sClasificacion'] <> 'Tiempo en Operacion' then
  begin
...
procedure TfrmBitacoraDepartamental_Tierra.mDescripcionStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
begin
    AStyle := cxNormal;
...
procedure TfrmBitacoraDepartamental_Tierra.nxNumGerencialChange(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.tsNumeroActividadChange(
  Sender: TObject);
begin
  ...
  if (frmBarra1.btnCancel.Enabled = True) and (not tsNumeroActividad.ReadOnly) then
    ...
    begin
      ...
      ActividadesIguales.ParamByName('fecha').AsDate := tdIdFecha.Date;
      ActividadesIguales.Open;
      if ActividadesIguales.FieldValues['dPonderado'] <> null then
        tdPonderado.Value := ActividadesIguales.FieldValues['dPonderado']
      else
        tdPonderado.Value := 0;
...
procedure TfrmBitacoraDepartamental_Tierra.Actividades1Click(Sender: TObject);
begin
   // GeneraReporteDiario_PDF(FtAbordo,FtsActividad);
end;

procedure TfrmBitacoraDepartamental_Tierra.ActividadesIgualesAfterScroll(
  DataSet: TDataSet);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.QryBitacoraCalcFields(
  DataSet: TDataSet);
begin
  try
    ...
    connection.QryBusca.Active := False;
    connection.QryBusca.SQL.Clear;
    connection.QryBusca.SQL.Add('select SUM(dAvance) as Avance from bitacoradeactividades where sContrato =:Contrato ' +
              'and dIdFecha =:Fecha and sNumeroOrden =:Orden and sNumeroActividad =:Actividad and sWbs =:Wbs group by sContrato ');
    connection.QryBusca.ParamByName('contrato').AsString  := param_global_contrato;
    ...
    begin
      ...
      connection.QryBusca.Params.ParamByName('contrato').Value := param_global_contrato;
      connection.QryBusca.Params.ParamByName('orden').DataType := ftString;
      connection.QryBusca.Params.ParamByName('orden').Value := Param_Frente;
      connection.QryBusca.Params.ParamByName('turno').DataType := ftString;
      connection.QryBusca.Params.ParamByName('turno').Value := global_turno_reporte;
      connection.QryBusca.Open;
      if connection.QryBusca.RecordCount > 0 then
...
procedure TfrmBitacoraDepartamental_Tierra.qryNotasGerencialAfterScroll(
  DataSet: TDataSet);
begin
    if qryNotasGerencial.RecordCount > 0 then
    begin
        ...
        cbbTipoActividad.Text := DataSet.FieldByName( 'eTipoActividad' ).AsString;

        if QryNOtasGerencial.FieldByName( 'sIdTipoMovimiento' ).AsString='EN' then
        begin
          LCmbCortes.Visible:=True;
          lblCortes.Visible:=True;
          LCmbCortes.EditValue:=QryNOtasGerencial.FieldByName('iHermano').asinteger;
          {if QrNotasCortes.Locate('iIdActividad',QryNOtasGerencial.FieldByName('iHermano').AsInteger,[]) then
            LCmbCortes.EditValue:=QrNotasCortes.FieldByName('nota').AsString
          else
            LCmbCortes.EditValue:='';}
        end
        else
        begin
          LCmbCortes.Visible:=false;
          lblCortes.Visible:=false;
        end;
    end;
end;

procedure TfrmBitacoraDepartamental_Tierra.QrFrentesAfterScroll(DataSet: TDataSet);
begin
    ...
    if QrFrentes.FieldByName('Convenio').AsString = '-1' then
      lblFolio.Caption := '[F O L I O : '+ QrFrentes.FieldByName('sNumeroOrden').AsString + ']' +'     '+'[  REPROG. SIN REPROGRAMACION ]'
    else
       lblFolio.Caption := '[F O L I O : '+ QrFrentes.FieldByName('sNumeroOrden').AsString + ']' +'     '+'[  REPROG. '+QrFrentes.FieldByName('Convenio').AsString+' ]';
    LeerDatasets(QrFrentes.FieldByName('sContrato').AsString,QrFrentes.FieldByName('sNumeroOrden').AsString);

    //Informacion del folio
    if QrFrentes.FieldByName('Convenio').AsString = '-1' then
       GrdOrden.Hint := 'NO REPROGRAMADO'
    else
       GrdOrden.Hint := QrFrentes.FieldByName('Convenio').AsString +': DEL '+DateToStr(QrFrentes.FieldByName('FechaInicio').AsDateTime)+' AL '+DateToStr(QrFrentes.FieldByName('FechaFinal').AsDateTime);
...
procedure TfrmBitacoraDepartamental_Tierra.QryBitacoraAfterScroll(
  DataSet: TDataSet);
var
...
begin
    ...
    begin
        ...
        QryNotasGerencial.SQL.Add('select iIdDiario, mDescripcion, sHoraInicio, sHoraFinal, sConceptoGerencial, sIdClasificacion, dCantidad, format(dAvance,2) as dAvance, sWbs, '+
                                  'concat(sHoraInicio," - ",sHoraFinal) as Nota, lImprime,iHermano,iIdTarea,iIdActividad, eTipoActividad,sIdTipoMovimiento '+
                                  'from bitacoradeactividades where sContrato =:Contrato and sIdConvenio =:Convenio '+
                                  'and dIdFecha =:Fecha && snumeroorden = :folio && iIddiarioNota =:diario && Find_In_Set(sIdTipoMovimiento,"ED,EN") order by sHoraInicio, sHoraFinal ');
        ...
        QryNotasGerencial.ParamByName('diario').asInteger   := QryBitacora.fieldbyname('iIdDiario').AsInteger;
        QryNotasGerencial.Open;

        QrNotasCortes.Active := false;
        QrNotasCortes.SQL.Clear;
        QrNotasCortes.SQL.Add('select iIdDiario, mDescripcion, sHoraInicio, sHoraFinal, sConceptoGerencial, sIdClasificacion, dCantidad, format(dAvance,2) as dAvance, sWbs, '+
                                  'concat(sHoraInicio," - ",sHoraFinal) as Nota, lImprime,iHermano,iIdTarea,iIdActividad, eTipoActividad '+
                                  'from bitacoradeactividades where sContrato =:Contrato and sIdConvenio =:Convenio '+
                                  'and dIdFecha =:Fecha && snumeroorden = :folio && iIddiarioNota =:diario && sIdTipoMovimiento = "ED" order by sHoraInicio, sHoraFinal ');
        QrNotasCortes.ParamByName('Contrato').AsString  := param_global_Contrato;
        QrNotasCortes.ParamByName('Convenio').AsString  := QrFrentes.FieldByName('Convenio').AsString;
        QrNotasCortes.ParamByName('Fecha').AsDate       := tdIdFecha.Date;
        QrNotasCortes.ParamByName('folio').asstring     := QrFrentes.FieldByName('snumeroorden').asstring;
        QrNotasCortes.ParamByName('diario').asInteger   := QryBitacora.fieldbyname('iIdDiario').AsInteger;
        QrNotasCortes.Open;
...
procedure TfrmBitacoraDepartamental_Tierra.frmBarra1btnPrinterClick(
  Sender: TObject);
begin
   // GeneraReporteDiario_PDF(FtAbordo,FtsActividad)
end;

procedure TfrmBitacoraDepartamental_Tierra.rDiarioGetValue(
...
begin
  ...
  If CompareText(VarName, 'SUPERINTENDENTE') = 0 then
      If frmDiarioTurno.ReporteDiario.FieldValues['sOrigenTierra'] = 'No' Then
...
procedure TfrmBitacoraDepartamental_Tierra.CopiaMemo(Sender: TObject);
begin
  tmDescripcion.Text := (Sender as tMemo).Text;
end;

procedure TfrmBitacoraDepartamental_Tierra.CosolidarNotasGerenciales1Click(
  Sender: TObject);
var
   sConcepto   : string;
   Diario      : integer;
...
procedure TfrmBitacoraDepartamental_Tierra.cxAbiertoClick(Sender: TObject);
begin
        chkEstatusHora.OnEnter(sender);
end;

procedure TfrmBitacoraDepartamental_Tierra.cxAceptarClick(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.cxButton1Click(Sender: TObject);
var
  Form : TForm;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.cxButton2Click(Sender: TObject);
var
...
procedure TfrmBitacoraDepartamental_Tierra.cxButton3Click(Sender: TObject);
var
  c : Integer;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.cxCancelarClick(Sender: TObject);
begin
    PanelPassword.Visible := False;
end;

procedure TfrmBitacoraDepartamental_Tierra.cxCerradoClick(Sender: TObject);
begin
    chkEstatusHora.OnEnter(sender);
end;

procedure TfrmBitacoraDepartamental_Tierra.cxComboBox1PropertiesChange(
  Sender: TObject);
var
   sHora : string;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.cxEditaClick(Sender: TObject);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.dAvanceStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
begin
    AStyle := cxNormal;
...
procedure TfrmBitacoraDepartamental_Tierra.GrdOrdenCellClick(Column: TColumn);
begin
     if QryBitacora.RecordCount > 0 then
         tdAvanceGlobal.Value := AvanceFolio;
end;

procedure TfrmBitacoraDepartamental_Tierra.GrdOrdenDblClick(Sender: TObject);
begin

  frmBarra1btnAddClick(sender);
end;

procedure TfrmBitacoraDepartamental_Tierra.GrdOrdenDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  if GrdOrden.DataSource.DataSet.FieldByName('Orden').AsString = 'Si' then
  begin
    GrdOrden.Canvas.Font.Color:=esColor(12);
    GrdOrden.Canvas.Font.Style := [fsBold];
    GrdOrden.Canvas.Brush.Color :=  $00F7F5EE;
  end;
...
procedure TfrmBitacoraDepartamental_Tierra.DesagruparHorarios1Click(Sender: TObject);
begin
    connection.zCommand.Active := False;
    connection.zCommand.SQL.Clear;
...
procedure TfrmBitacoraDepartamental_Tierra.ds_bitacoradeequiposDataChange(
...
procedure TfrmBitacoraDepartamental_Tierra.ActividadesIgualesCalcFields(
  DataSet: TDataSet);
begin
...
procedure TfrmBitacoraDepartamental_Tierra.ActualizaIdDiario(dParamContrato: string; dParamFecha: TDate; dParamIdDiario, dParamIdDiarioOld: Integer);
var
...
procedure TfrmBitacoraDepartamental_Tierra.OrdenarNotas(sParamOrden: string);
var
...
procedure TfrmBitacoraDepartamental_Tierra.ActualizaImprime;
var
...
procedure TfrmBitacoraDepartamental_Tierra.AdvGlassButton1Click(Sender: TObject);
begin
    if gridNotas.Top = 263 then
    begin
        gridNotas.Top   := 223;
        gridNotas.Width := 243;
        gridNotas.Height:= 188;
    end
    else
    begin
        gridNotas.Top   := 263;
        gridNotas.Width := 243;
        gridNotas.Height:= 148;
    end;
end;

procedure TfrmBitacoraDepartamental_Tierra.AdvNotaClick(Sender: TObject);
begin
   if advNota.Caption = 'N' then
   begin
      tmDescripcion.Color := $00E6FEFF;
   ...
   begin
      ...
      advNota.Hint := 'Nota Actividad';
      advNota.Caption := 'N';
   end;
end;

procedure TfrmBitacoraDepartamental_Tierra.AgruparHorarios1Click(Sender: TObject);
var
  SavePlace: TBookmark;
  iGrid: integer;
...
procedure TfrmBitacoraDepartamental_Tierra.formatoEncabezado;
begin
...
function TfrmBitacoraDepartamental_Tierra.CompararFechas(fecha1: TDate; fecha2: TDate) : Boolean;
var
...
procedure TfrmBitacoraDepartamental_Tierra.CargaActividades;
begin
...
function TfrmBitacoraDepartamental_Tierra.ValidaReporteDiario;
begin
...
function TfrmBitacoraDepartamental_Tierra.AvanceActual(sParamActividad: string; sParamWbs: string): double;
var
   dAvanceAnterior,
...
procedure TfrmBitacoraDepartamental_Tierra.MantenerHorario1Click(Sender: TObject);
begin
    connection.zCommand.Active := False;
    connection.zCommand.SQL.Clear;
    connection.zCommand.SQL.Add('UPDATE bitacoradepersonal SET sHoraInicioG = sHoraInicio, sHoraFinalG = sHoraFinal ' +
...
function TfrmBitacoraDepartamental_Tierra.MaximoItem;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.PartidaExistente(sParamActividad: string; sParamWbs: string; sParamTE: string);
begin
...
function TfrmBitacoraDepartamental_Tierra.InstaladoOrden(sParamActividad: string; sParamWbs: string; sParamFecha: Integer) :double;
begin
...
procedure TfrmBitacoraDepartamental_Tierra.ItmRecursosClick(Sender: TObject);
begin
   FrmRecursosTierra:= TFrmRecursosTierra.MiCreate(nil,param_global_Contrato,QrFrentes.FieldByName('Convenio').AsString,
                          QrFrentes.FieldByName('snumeroorden').asstring,tdIdFecha.Date);
   try
    FrmRecursosTierra.ShowModal;
   finally
     FrmRecursosTierra.Destroy;
   end;
end;

procedure TfrmBitacoraDepartamental_Tierra.lblFolioDblClick(Sender: TObject);
begin
    Application.CreateForm( TfrmReprogramacionFolio, frmReprogramacionFolio );
    frmReprogramacionFolio.ShowModal;
end;

function TfrmBitacoraDepartamental_Tierra.CantidadAnexoC;
begin
    Connection.qryBusca.Active := False;
    Connection.qryBusca.SQL.Clear;
...
function TfrmBitacoraDepartamental_Tierra.InstaladoAnexoC(sParamActividad: string; sParamWbs: string): double;
begin
    connection.QryBusca.Active := False;
    connection.QryBusca.SQL.Clear;
    connection.QryBusca.SQL.Add('Select Sum(dCantidad) as dCantidad from bitacoradeactividades where ' +
...
Function TfrmBitacoraDepartamental_Tierra.AvanceFolio;
var
...
procedure TfrmBitacoraDepartamental_Tierra.chkEstatusHoraEnter(Sender: TObject);
begin
   if chkEstatusHora.Checked then
   begin
       cxAbierto.Visible      := True;
       cxCerrado.Visible      := False;
       chkEstatusHora.Checked := False;
   end
   else
   begin
       cxAbierto.Visible      := False;
...
procedure TfrmBitacoraDepartamental_Tierra.Minimo_id;
begin
...
Function TfrmBitacoraDepartamental_Tierra.SumaMaterial(sParamMaterial: string) : Double;
begin
    connection.QryBusca.Active := False;
    connection.QryBusca.SQL.Clear;
...
Function TfrmBitacoraDepartamental_Tierra.MaterialDisponible(sParamMaterial: string) : Double;
begin
    connection.QryBusca.Active := False;
    connection.QryBusca.SQL.Clear;
