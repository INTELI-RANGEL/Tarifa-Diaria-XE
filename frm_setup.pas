...
uses
  ...
  ZDataset, ZAbstractRODataset, ZAbstractDataset, UnitExcepciones,
  UnitTIniTracer, NxPageControl, AdvGroupBox, AdvListV, AdvPanel,
  AdvOfficeButtons, DBAdvOfficeButtons, JvExMask, JvToolEdit, JvCombobox,
  AdvGlowButton, UnitValidacion, Menus,StrUtils, cxGraphics,
...
type
  TfrmSetup = class(TForm)
    ...
    tsGalon: TDBEdit;
    lbl1: TLabel;
    DbCmbAvJornada: TDBComboBox;
    tsTelefono: TDBEdit;
    Label123: TLabel;
    DBComboBox4: TDBComboBox;
    AdvGroupBox37: TAdvGroupBox;
    Image1: TImage;
    AdvGroupBox38: TAdvGroupBox;
    Image2: TImage;
    OpenPictureAux1: TOpenPictureDialog;
    OpenPictureAux2: TOpenPictureDialog;
    AdvGroupBox39: TAdvGroupBox;
    Image3: TImage;
    OpenPictureAux3: TOpenPictureDialog;
    AdvGroupBox40: TAdvGroupBox;
    Image4: TImage;
    OpenPictureAux4: TOpenPictureDialog;
    OpenPictureLibros: TOpenPictureDialog;
    Label124: TLabel;
    DBComboBox5: TDBComboBox;
    dbchkSubtotales: TDBCheckBox;
    Label125: TLabel;
    tdHojas: TDBComboBox;
    AdvGroupBox41: TAdvGroupBox;
    Label126: TLabel;
    cmbSecciones: TJvCheckedComboBox;
...
//     Shift: TShiftState);
    ...
    procedure cmbFormatoCEnter(Sender: TObject);
    procedure cmbFormatoCExit(Sender: TObject);
    ...
    procedure tsLineaKeyPress(Sender: TObject; var Key: Char);
    ...
    procedure DbCmbAvJornadaEnter(Sender: TObject);
    procedure DbCmbAvJornadaExit(Sender: TObject);
...
uses frxDsgnIntf,frxClass, frm_inteligent;

{$R *.dfm}

procedure TfrmSetup.Image1Click(Sender: TObject);
begin
  if (configuracion.State = dsEdit) then
  begin
    OpenPictureAux1.Title := 'Inserta Imagen';
    if OpenPictureAux1.Execute then
    begin
      try
        TImage(Sender).Picture.LoadFromFile(OpenPictureAux1.FileName);
      except
...
procedure TfrmSetup.Image2Click(Sender: TObject);
begin
  if (configuracion.State = dsEdit) then
  begin
    OpenPictureAux2.Title := 'Inserta Imagen';
    if OpenPictureAux2.Execute then
    begin
      try
        TImage(Sender).Picture.LoadFromFile(OpenPictureAux2.FileName);
      except
//        TImage(Sender).Picture.LoadFromFile('');
      end
    end
...
procedure TfrmSetup.Image3Click(Sender: TObject);
begin
  if (configuracion.State = dsEdit) then
  begin
    OpenPictureAux3.Title := 'Inserta Imagen';
    if OpenPictureAux3.Execute then
    begin
      try
        TImage(Sender).Picture.LoadFromFile(OpenPictureAux3.FileName);
      except
//        TImage(Sender).Picture.LoadFromFile('');
      end
    end
...
procedure TfrmSetup.Image4Click(Sender: TObject);
begin
  if (configuracion.State = dsEdit) then
  begin
    OpenPictureAux4.Title := 'Inserta Imagen';
    if OpenPictureAux4.Execute then
    begin
      try
        TImage(Sender).Picture.LoadFromFile(OpenPictureAux4.FileName);
      except
//        TImage(Sender).Picture.LoadFromFile('');
      end
    end
...
procedure TfrmSetup.Image5Click(Sender: TObject);
begin
  if (configuracion.State = dsEdit) then
  begin
    OpenPictureLibros.Title := 'Inserta Imagen';
    if OpenPictureLibros.Execute then
    begin
      try
        TImage(Sender).Picture.LoadFromFile(OpenPictureLibros.FileName);
      except
//        TImage(Sender).Picture.LoadFromFile('');
      end
    end
...
begin
  ...
  begin
    ...
    if configuracion.FieldByName('sConcentradoP').AsString = '02' then
      dbchkSubtotales.Visible := True
    else
      dbchkSubtotales.Visible := False;
    ...
    begin
      ...
      BlobField := configuracion.FieldByName('bImagenAux1');
      BS := configuracion.CreateBlobStream(BlobField, bmRead);
      if bs.Size > 1 then
      begin
        try
...
//        Image1.Picture.LoadFromFile('');
      ...
      BlobField := configuracion.FieldByName('bImagenAux2');
      BS := configuracion.CreateBlobStream(BlobField, bmRead);
      if bs.Size > 1 then
      begin
        try
...
//        Image2.Picture.LoadFromFile('');
      ...
      BlobField := configuracion.FieldByName('bImagenAux3');
      BS := configuracion.CreateBlobStream(BlobField, bmRead);
      if bs.Size > 1 then
      begin
        try
...
//        Image3.Picture.LoadFromFile('');
      ...
      BlobField := configuracion.FieldByName('bImagenAux4');
      BS := configuracion.CreateBlobStream(BlobField, bmRead);
      if bs.Size > 1 then
      begin
        try
...
//        Image4.Picture.LoadFromFile('');
 ...
 (* Almacen.Visible := False;
  ...
  IniciaSecciones;
  CargarCheckCombo(configuracion.FieldbyName('sSeccionImprime').AsString,CmbSecciones);
...
procedure TfrmSetup.ListView1Click(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    if OpenPictureAux1.FileName <> '' then
    begin
      try
        BlobField := configuracion.FieldByName('bImagenAux1');
        BS := configuracion.CreateBlobStream(BlobField, bmWrite);
        try
          Pic := TJpegImage.Create;
          try
            Pic.LoadFromFile(OpenPictureAux1.FileName);
            Pic.SaveToStream(Bs);
          finally
            Pic.Free;
          end;
    ...
    if OpenPictureAux2.FileName <> '' then
    begin
      try
        BlobField := configuracion.FieldByName('bImagenAux2');
        BS := configuracion.CreateBlobStream(BlobField, bmWrite);
    ...
    if OpenPictureAux3.FileName <> '' then
    begin
      try
        BlobField := configuracion.FieldByName('bImagenAux3');
        BS := configuracion.CreateBlobStream(BlobField, bmWrite);
        try
          Pic := TJpegImage.Create;
          try
            Pic.LoadFromFile(OpenPictureAux3.FileName);
            Pic.SaveToStream(Bs);
          finally
            Pic.Free;
          end;
    ...
    if OpenPictureAux4.FileName <> '' then
    begin
      try
        BlobField := configuracion.FieldByName('bImagenAux4');
        BS := configuracion.CreateBlobStream(BlobField, bmWrite);
        try
          Pic := TJpegImage.Create;
          try
            Pic.LoadFromFile(OpenPictureAux4.FileName);
            Pic.SaveToStream(Bs);
          finally
            Pic.Free;
          end;
...
//    configuracion.FieldByName('sPasswordPdf').AsString:='';
    ...
    configuracion.FieldByName('sSeccionImprime').AsString:=CmbSecciones.Text;
    configuracion.Post;
...
begin
  cmbcalculatm.Color:=global_color_entrada
end;

procedure TfrmSetup.CmbCalculaTMExit(Sender: TObject);
begin
  cmbcalculatm.color:=global_color_salida
end;
...
procedure TfrmSetup.cmbFormatoCEnter(Sender: TObject);
begin
  cmbformatoc.Color:=global_color_entrada
end;

procedure TfrmSetup.cmbFormatoCExit(Sender: TObject);
begin
  cmbformatoc.color:=global_color_salida
end;
    ...
    if not (Menu.Parent is TMenuItem) then begin
      ...
      begin
        ...
        begin
          qry.Active := false;
          qry.SQL.Clear;
          qry.SQL.Add('insert into programas (sIdPrograma,sDescripcion, sIdIdioma) values (:modulo,:descripcion, "ESP")' +
...
begin
  tsTipoContrato.Color := global_color_salida;
...
begin
      tsReporteBarco.Color := global_color_entrada
end;

procedure TfrmSetup.tsReporteBarcoExit(Sender: TObject);
begin
      tsReporteBarco.Color := global_color_salida
end;
...
begin
    tsRepresentante.Color := global_Color_entrada;
end;

procedure TfrmSetup.tsRepresentanteExit(Sender: TObject);
begin
    tsRepresentante.Color := global_Color_salida;
end;
...
begin
  tlAsistencia.Color := global_color_salida;
...
begin
  tsestimacionesgenerales.color:=global_color_entrada
end;

procedure TfrmSetup.tsEstimacionesGeneralesExit(Sender: TObject);
begin
  tsestimacionesgenerales.color:=global_color_salida
end;
...
begin
  tlImprimeExtraordinario.Color := global_color_entrada
end;

procedure TfrmSetup.tlImprimeExtraordinarioExit(Sender: TObject);
begin
  tlImprimeExtraordinario.Color := global_color_salida
end;
...
begin
  tsIdConvenio.Color := global_color_Entrada
end;

procedure TfrmSetup.tsIdConvenioExit(Sender: TObject);
begin
  tsIdConvenio.Color := global_color_salida
end;
...
begin
  tiRedondeoMaterial.Color := global_color_entrada
end;

procedure TfrmSetup.tiRedondeoMaterialExit(Sender: TObject);
begin
  tiRedondeoMaterial.Color := global_color_salida
end;
...
begin
  tiRedondeoEquipo.Color := global_color_entrada
end;

procedure TfrmSetup.tiRedondeoEquipoExit(Sender: TObject);
begin
  tiRedondeoEquipo.Color := global_color_salida
end;
...
begin
  tiRedondeoPersonal.Color := global_color_entrada
end;

procedure TfrmSetup.tiRedondeoPersonalExit(Sender: TObject);
begin
  tiRedondeoPersonal.Color := global_color_salida
end;
...
begin
  tiRedondeoEmbarcacion.Color := global_color_entrada
end;

procedure TfrmSetup.tiRedondeoEmbarcacionExit(Sender: TObject);
begin
  tiRedondeoEmbarcacion.Color := global_color_salida
end;
...
begin
  tsPartidaEfectiva.Color := global_color_entrada
end;

procedure TfrmSetup.tsPartidaEfectivaExit(Sender: TObject);
begin
  tsPartidaEfectiva.Color := global_color_salida
end;
...
procedure TfrmSetup.tsLineaKeyPress(Sender: TObject; var Key: Char);
begin
     if Key = #13 then
        cmbFormatoC.SetFocus
end;
...
begin
    tsImpresionPaquetes.color := global_color_entrada
end;

procedure TfrmSetup.tsImpresionPaquetesExit(Sender: TObject);
begin
    tsImpresionPaquetes.color := global_color_salida
end;
...
begin
  tsImprimePEP.Color := global_color_entrada
end;

procedure TfrmSetup.tsImprimePEPExit(Sender: TObject);
begin
  tsImprimePEP.Color := global_color_salida
end;
...
begin
  tsBaseCalculo.Color := global_color_entrada
end;

procedure TfrmSetup.tsBaseCalculoExit(Sender: TObject);
begin
  tsBaseCalculo.Color := global_color_salida
end;
...
begin
  tsAvanceBitacora.Color := global_color_Entrada
end;

procedure TfrmSetup.tsAvanceBitacoraExit(Sender: TObject);
begin
  tsAvanceBitacora.Color := global_color_salida
end;
...
begin
      tsGeneradoresBarco.Color := global_color_entrada
end;

procedure TfrmSetup.tsGeneradoresBarcoExit(Sender: TObject);
begin
    tsGeneradoresBarco.Color := global_color_salida
end;
...
begin
      tsGeneradores.Color := global_color_entrada
end;

procedure TfrmSetup.tsGeneradoresExit(Sender: TObject);
begin
      tsGeneradores.Color := global_color_salida
end;
...
begin
  tsOrdenPersonal.Color := global_color_entrada
end;

procedure TfrmSetup.tsOrdenPersonalExit(Sender: TObject);
begin
  tsOrdenPersonal.Color := global_color_salida
end;

procedure TfrmSetup.tsTipoAjusteCostoEnter(Sender: TObject);
begin
  tsTipoAjusteCosto.Color := global_color_entrada
end;

procedure TfrmSetup.tsTipoAjusteCostoExit(Sender: TObject);
begin
  tsTipoAjusteCosto.Color := global_color_salida
end;
...
begin
  tsTipoGeneracion.Color := global_color_entrada
end;

procedure TfrmSetup.tsTipoGeneracionExit(Sender: TObject);
begin
  tsTipoGeneracion.Color := global_color_salida
end;
...
begin
  tsBaseGeneracion.Color := global_color_entrada
end;

procedure TfrmSetup.tsBaseGeneracionExit(Sender: TObject);
begin
  tsBaseGeneracion.Color := global_color_salida
end;
...
begin
  tsViewIsometrico.Color := global_color_entrada
end;

procedure TfrmSetup.tsViewIsometricoExit(Sender: TObject);
begin
  tsViewIsometrico.Color := global_color_salida
end;
...
begin
  tsFotografias.Color := global_color_entrada
end;

procedure TfrmSetup.tsFotografiasExit(Sender: TObject);
begin
  tsFotografias.Color := global_color_salida
end;
...
begin
  tlComentariosReporte.Color := global_color_entrada
end;

procedure TfrmSetup.tlComentariosReporteExit(Sender: TObject);
begin
  tlComentariosReporte.Color := global_color_salida
end;
...
begin
  tlIncluyeGrafica.Color := global_color_entrada
end;

procedure TfrmSetup.tlIncluyeGraficaExit(Sender: TObject);
begin
  tlIncluyeGrafica.Color := global_color_salida
end;
...
begin
  tiFirmas.Color := global_color_entrada
end;

procedure TfrmSetup.tiFirmasExit(Sender: TObject);
begin
  tiFirmas.Color := global_color_salida
end;
...
begin
      tiFirmasReportes.Color := global_color_entrada
end;

procedure TfrmSetup.tiFirmasReportesExit(Sender: TObject);
begin
      tiFirmasReportes.Color := global_color_salida
end;
...
begin
  tsFirmasElectronicas.Color := global_Color_Entrada
end;

procedure TfrmSetup.tsFirmasElectronicasExit(Sender: TObject);
begin
  tsFirmasElectronicas.Color := global_Color_salida
end;
...
begin
  tsTipsInicial.Color := global_Color_entrada
end;

procedure TfrmSetup.tsTipsInicialExit(Sender: TObject);
begin
  tsTipsInicial.Color := global_Color_salida
end;
...
begin
  tsImporteRetencion.Color := global_color_entrada
end;

procedure TfrmSetup.tsImporteRetencionExit(Sender: TObject);
begin
  tsImporteRetencion.Color := global_color_salida
end;

procedure TfrmSetup.tsTipoEstimacionEnter(Sender: TObject);
begin
  tsTipoEstimacion.Color := global_color_entrada
end;

procedure TfrmSetup.tsTipoEstimacionExit(Sender: TObject);
begin
  tsTipoEstimacion.Color := global_color_salida
end;
...
begin
  tsTerminoPenalizacion.Color := global_color_Entrada
end;

procedure TfrmSetup.tsTerminoPenalizacionExit(Sender: TObject);
begin
  tsTerminoPenalizacion.Color := global_color_salida
end;
...
begin
  tlIncluyeAvanceOrdenes.Color := global_color_entrada
end;

procedure TfrmSetup.tlIncluyeAvanceOrdenesExit(Sender: TObject);
begin
  tlIncluyeAvanceOrdenes.Color := global_color_salida
end;

procedure TfrmSetup.tlIncluyeAvanceContratoEnter(Sender: TObject);
begin
  tlIncluyeAvanceContrato.Color := global_color_entrada
end;

procedure TfrmSetup.tlIncluyeAvanceContratoExit(Sender: TObject);
begin
  tlIncluyeAvanceContrato.Color := global_color_salida
end;
...
begin
  tsSeguridadGenerador.Color := global_color_entrada
end;

procedure TfrmSetup.tsSeguridadGeneradorExit(Sender: TObject);
begin
  tsSeguridadGenerador.Color := global_color_salida
end;
...
begin
  ssimbolo.color:=global_color_entrada
end;

procedure TfrmSetup.sSimboloExit(Sender: TObject);
begin
 ssimbolo.color:= global_color_salida
end;
...
begin
   dbUtilidad.Color := Global_Color_Entrada ;
end;

procedure TfrmSetup.dbUtilidadExit(Sender: TObject);
begin
  dbutilidad.Color := Global_Color_Salida ;
end;
...
begin
  dbPerforacion.Color := global_color_entrada;
end;
...
begin
  dbCargo1.Color := Global_Color_Entrada ;
end;

procedure TfrmSetup.dbCargo1Exit(Sender: TObject);
begin
    dbCargo1.Color := Global_Color_Salida ;
end;
...
begin
    dbCargo2.Color := Global_Color_Entrada ;
end;

procedure TfrmSetup.dbCargo2Exit(Sender: TObject);
begin
   dbCargo2.Color := Global_Color_Salida ;
end;
...
procedure TfrmSetup.DbCmbAvJornadaEnter(Sender: TObject);
begin
  DbCmbAvJornada.Color:=global_color_entrada;
end;

procedure TfrmSetup.DbCmbAvJornadaExit(Sender: TObject);
begin
  DbCmbAvJornada.Color:=global_color_salida;
end;
...
procedure TfrmSetup.DBComboBox5Change(Sender: TObject);
begin
  if DBComboBox5.Text = '02' then
    dbchkSubtotales.Visible := True
  else
    dbchkSubtotales.Visible := False;
...
begin
  dbexplosion.color:=global_color_entrada
end;

procedure TfrmSetup.dbExplosionExit(Sender: TObject);
begin
  dbexplosion.Color:=global_color_salida
end;
...
begin
  dbFinanciamiento.Color := Global_Color_Entrada ;
end;

procedure TfrmSetup.dbFinanciamientoExit(Sender: TObject);
begin
  dbFinanciamiento.Color := Global_Color_Salida ;
end;
...
begin
 dbIndirecto.Color := Global_Color_Entrada ;
end;

procedure TfrmSetup.dbIndirectoExit(Sender: TObject);
begin
   dbIndirecto.Color := Global_Color_Salida;
end;
...
begin
      dbMailAutoriza.Color := global_color_entrada
end;

procedure TfrmSetup.dbMailAutorizaExit(Sender: TObject);
begin
      dbMailAutoriza.Color := global_color_salida
end;
...
begin
  dbMostrar.Color := Global_Color_Entrada;
end;

procedure TfrmSetup.dbMostrarExit(Sender: TObject);
begin
  dbMostrar.Color := Global_Color_Salida;
end;
...
procedure TfrmSetup.IniciaSecciones;
var
   cadena, sItem : string;
   lContinua : Boolean;
   numero    : integer;
begin
   ...
   cadena := configuracion.FieldByName('sSecciones').AsString;
   lContinua := True;
   while lContinua do
   begin
       numero := pos(',',cadena);
       if numero > 0 then
       begin
          sItem     := copy(cadena,0,numero -1);
          cadena    := copy(cadena,numero + 1, length(cadena));
          lContinua := True;
       end
       else
       begin
