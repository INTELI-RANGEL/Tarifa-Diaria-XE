unit frm_DesgloceActividadesPEQ;
...
uses
  ...
  cxLookupEdit, cxDBLookupEdit, cxDBExtLookupComboBox, cxCalendar, dxSkinBlack,
...
type
  TfrmDesgloceActividadesPEQ = class(TForm)
    ...
    cxEquipo: TcxGrid;
    cxEquipoView1: TcxGridDBTableView;
    eFecha: TcxGridDBColumn;
    eInicio: TcxGridDBColumn;
    eFin: TcxGridDBColumn;
    ePartida: TcxGridDBColumn;
    eCantidad: TcxGridDBColumn;
    eJornada: TcxGridDBColumn;
    eDescripcion: TcxGridDBColumn;
    cxGridLevel1: TcxGridLevel;
    cxPersonal: TcxGrid;
    cxPersonalView2: TcxGridDBTableView;
    pFecha: TcxGridDBColumn;
    pPartida: TcxGridDBColumn;
    pCantidad: TcxGridDBColumn;
    pInicio: TcxGridDBColumn;
    pFin: TcxGridDBColumn;
    pJornada: TcxGridDBColumn;
    pDescripcion: TcxGridDBColumn;
    cxGridLevel2: TcxGridLevel;
    ds_personal: TDataSource;
    zPersonal: TZQuery;
    ds_equipos: TDataSource;
    zEquipos: TZQuery;
    pActividad: TcxGridDBColumn;
    eActividad: TcxGridDBColumn;
    cxPersonalView2Column1: TcxGridDBColumn;
    cxEquipoView1Column1: TcxGridDBColumn;
...
var
  frmDesgloceActividadesPEQ: TfrmDesgloceActividadesPEQ;
...
procedure TfrmDesgloceActividadesPEQ.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := cafree;
end;

procedure TfrmDesgloceActividadesPEQ.FormShow(Sender: TObject);
begin
   ...
   begin
       ...
       zqActividades.ParamByName('Folio').AsString    := global_FrenteTrabajo;
       ...
       zPersonal.ParamByName('Orden').AsString    := global_Contrato;
       zPersonal.ParamByName('Folio').AsString    := global_FrenteTrabajo;
       zPersonal.ParamByName('fechaI').AsDate     := connection.zCommand.FieldByName('minima').AsDateTime;
       zPersonal.ParamByName('fechaF').AsDate     := connection.zCommand.FieldByName('maxima').AsDateTime;
       zPersonal.Open;

       zEquipos.Active := False;
       zEquipos.ParamByName('contrato').AsString := global_Contrato_barco;
       zEquipos.ParamByName('Orden').AsString    := global_Contrato;
       zEquipos.ParamByName('Folio').AsString    := global_FrenteTrabajo;
       zEquipos.ParamByName('fechaI').AsDate     := connection.zCommand.FieldByName('minima').AsDateTime;
       zEquipos.ParamByName('fechaF').AsDate     := connection.zCommand.FieldByName('maxima').AsDateTime;
       zEquipos.Open;

   end;

   frmDesgloceActividadesPEQ.Caption := 'Integracion Nota Campo Actividades P y EQ  > Folio ['+zqActividades.FieldByName('sIdFolio').AsString+']';

end;

procedure TfrmDesgloceActividadesPEQ.cxButton1Click(Sender: TObject);
var
   QueryImagen: TZQuery;
begin
  try
    ...
    if pgOpciones.ActivePageIndex = 0 then
       ExportExcelPersonalizado(QueryImagen,cxViewActividades,'Actividades','Actividades Diarias Folio >'+ global_FrenteTrabajo);

    if pgOpciones.ActivePageIndex = 1 then
    begin
       ExportExcelPersonalizado(QueryImagen,cxPersonalView2,'Personal','Personal Diario Folio > '+global_FrenteTrabajo);
       ExportExcelPersonalizado(QueryImagen,cxEquipoView1,'Equipo','Equipo Diario Folio > '+global_FrenteTrabajo);
    end;
...
procedure TfrmDesgloceActividadesPEQ.cxViewActividadesDblClick(Sender: TObject);
begin
...
procedure TfrmDesgloceActividadesPEQ.EditPaquetesEnter(Sender: TObject);
begin

end;

//Procedimiento para guardar datos del Rx,
function TfrmDesgloceActividadesPEQ.CalculaAvances;
var
...
procedure TfrmDesgloceActividadesPEQ.zqActividadesCalcFields(DataSet: TDataSet);
begin
...
procedure TfrmDesgloceActividadesPEQ.NotasGerencial(sParamContrato: string; sParamOrden: string; sParamInicio: string; sParamFinal: string; dParamFechaI: TDate; dParamFechaF: TDate);
var
