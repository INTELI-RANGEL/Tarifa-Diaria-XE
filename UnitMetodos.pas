...
interface
  ...
  uses
    ...
    ComObj, UnitExcel, ZSqlProcessor, cxGroupBox, cxProgressBar, cxTextEdit ;
...
implementation
  ...
  begin
    with connection do
    begin
      ZSentencia.Active := False;
      ZSentencia.SQL.Text := 'select Sentencia from sentencias where Tabla = :tabla && Nombre = :nombre && Tipo = :tipo;';
      ZSentencia.ParamByName( 'tabla' ).AsString := Tabla;
      ZSentencia.ParamByName( 'nombre' ).AsString := Nombre;
      ZSentencia.ParamByName( 'Tipo' ).AsString := FTIPO_SENTENCIA[ Integer( Tipo ) ];
      ZSentencia.Open;

      if ZSentencia.RecordCount = 0 then
      ...
      Result := ZSentencia.FieldByName( 'Sentencia' ).AsString;
