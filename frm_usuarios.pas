...
uses
  ...
  ZAbstractDataset, UnitTBotonesPermisos,
...
type
  TfrmUsuarios = class(TForm)
    ...
    procedure dbEnviaMailEnter(Sender: TObject);
...
uses frm_ConfiguraMail, Utilerias;
...
procedure TfrmUsuarios.tsNombreKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
...
procedure TfrmUsuarios.dbEnviaMailEnter(Sender: TObject);
begin
    dbEnviaMail.Color := global_color_entrada
end;

procedure TfrmUsuarios.dbEnviaMailExit(Sender: TObject);
begin
    dbEnviaMail.Color := global_color_salida
end;
...
begin
    tlActivo.Color := global_color_entrada
end;
...
begin
    tsIp.Color := global_color_Entrada
end;

procedure TfrmUsuarios.tsIpExit(Sender: TObject);
begin
    tsIp.Color := global_color_salida
end;
