...
uses
  ...
  Dialogs, global, db, Grids, DBGrids, frm_Connection, StdCtrls,
  DBCtrls, ComCtrls, Mask, Utilerias, masUtilerias, Menus, Buttons,
  ExtCtrls, frxClass, frxDBSet, RXDBCtrl, DateUtils,
  math, RxMemDS, ZAbstractRODataset, ZDataset, unitExcel, ComObj,
...
type
  TfrmAjustaAnexo = class(TForm)
    ...
    procedure imgNotasClick(Sender: TObject);
...
begin
  try
    ...
    if lError then begin
      ...
      if not FileExists(global_files + 'reporteValidacion.fr3') then
...
begin
  try
    ...
    if sOpcion = 'DT' then
      if not FileExists(global_files + 'Anexo DT.fr3') then
        showmessage('El archivo de reporte Anexo DT.fr3 no existe, notifique al administrador del sistema');

    if sOpcion = 'DE' then
      if not FileExists(global_files + 'Anexo DE.fr3') then
        showmessage('El archivo de reporte Anexo DE.fr3 no existe, notifique al administrador del sistema');

    if sOpcion = 'DEDLL' then
      if not FileExists(global_files + 'Anexo DE dll.fr3') then
        showmessage('El archivo de reporte Anexo DE DLL.fr3 no existe, notifique al administrador del sistema');
...
procedure TfrmAjustaAnexo.rxMensualKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  tdMonto.Color := global_color_entrada
end;

procedure TfrmAjustaAnexo.tdMontoExit(Sender: TObject);
begin
  tdMonto.Color := global_color_salida
end;
...
procedure TfrmAjustaAnexo.grid_actividadesKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #26 then
  begin
...
procedure TfrmAjustaAnexo.grid_actividadesMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  UtGrid.dbGridMouseMoveCoord(x, y);
end;

procedure TfrmAjustaAnexo.grid_actividadesMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  UtGrid.DbGridMouseUp(Sender, Button, Shift, X, Y);
end;

procedure TfrmAjustaAnexo.grid_actividadesTitleClick(Column: TColumn);
begin
...
procedure TfrmAjustaAnexo.grid_actividadesGetCellParams(Sender: TObject;
...
procedure TfrmAjustaAnexo.imgNotasClick(Sender: TObject);
begin
  ActividadesxAnexo.Active := False;
  ActividadesxAnexo.Open;
end;
