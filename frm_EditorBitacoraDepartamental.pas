...
uses RichEdit, ShellAPI, frm_connection, frm_bitacoradepartamental_2, frm_NotasGenerales;
...
begin
    try
       ...
       frmNotasGenerales.tmNotas.Text := Editor.Text;
       frmNotasGenerales.zq_NotasGenerales.Refresh;
    Except
    end;
end;
...
begin
    ...
    if sTituloVentana <> ' NOTAS GENERALES' then
    begin
        ...
        zqRegistro.Params.ParamByName('Diario').Value  := frmbitacoradepartamental_2.ds_notasGerencial.DataSet.FieldValues['iIdDiario'];
    end
    else
    begin
        zqRegistro.SQL.Add('select * from notas_generales where sContrato =:Contrato and dIdFecha =:Fecha and iId =:Diario ');
        zqRegistro.Params.ParamByName('Contrato').DataType := ftString ;
        zqRegistro.Params.ParamByName('Contrato').Value    := param_global_contrato;
        zqRegistro.Params.ParamByName('Fecha').DataType    := ftDate ;
        zqRegistro.Params.ParamByName('Fecha').Value       := frmNotasGenerales.tSeleccionarFecha.Date;
        zqRegistro.Params.ParamByName('Diario').DataType   := ftInteger ;
        zqRegistro.Params.ParamByName('Diario').Value      := frmNotasGenerales.ds_notasGenerales.DataSet.FieldValues['iId']
    end;
    zqRegistro.Open ;
    zqRegistro.Edit ;
    ...
    begin
        Editor.Text := frmNotasGenerales.tmNotas.Text;
    end;
