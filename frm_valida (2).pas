...
type
  TfrmValida = class(TForm)
    ...
    OrdenComprasDiasCredito: TIntegerField;
    OrdenComprasLugarEntrega: TStringField;
    OrdenComprasCargoFlete: TStringField;
    OrdenComprasAnticipo: TFloatField;
    OrdenComprasCantAnticipo: TFloatField;
    OrdenComprasDescuento: TFloatField;
    OrdenComprasGastoFlete: TFloatField;
              ...
              begin
                ...
                begin
                  ...
                  Connection.qryBusca.SQL.Add('Select Sum(g.dCantidad * a.dVentaMN) as dMontoMN, Sum(g.dCantidad * a.dVentaDLL) as dMontoDLL From estimacionxpartida g ' +
                    'inner join actividadesxorden ao on(ao.scontrato=g.scontrato and ao.sidconvenio=:Convenio and ao.snumeroorden=g.snumeroorden and g.swbs=ao.swbs) ' +

                    'INNER JOIN actividadesxanexo a ON (g.sContrato = a.sContrato And a.sIdConvenio =ao.sidconvenio And ao.swbscontrato = a.swbs and a.stipoactividad=ao.stipoactividad) ' +
                    'Where g.sContrato = :Contrato And g.sNumeroOrden = :Orden And g.sNumeroGenerador = :Generador And g.lEstima = "Si" and ao.sTipoActividad = "Actividad" ' +
    ...
    if Estimaciones.RecordCount > 0 then
      if lfnValidaGenerador(global_contrato, global_convenio, Estimaciones.FieldValues['sNumeroOrden'], Estimaciones.FieldValues['sNumeroGenerador'], frmValida) then
        if tsNumeroOrden.Text <> global_contrato then
          procCaratulaGenerador(global_contrato, Estimaciones.FieldValues['iNumeroEstimacion'], Estimaciones.FieldValues['sNumeroOrden'], Estimaciones.FieldValues['sNumeroGenerador'], global_convenio, frmValida, frGenerador.OnGetValue, True)
        else
          procCaratulaGenerador(global_contrato, Estimaciones.FieldValues['iNumeroEstimacion'], Estimaciones.FieldValues['sNumeroOrden'], Estimaciones.FieldValues['sNumeroGenerador'], global_convenio, frmValida, frGenerador.OnGetValue, False)
      else
