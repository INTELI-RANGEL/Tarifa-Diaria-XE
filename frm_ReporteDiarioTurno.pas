...
type
  TfrmDiarioTurno = class(TForm)
    ...
    procedure cxAjustesClick(Sender: TObject);
...
uses frm_inteligent, frm_bitacoraxalcance, frm_bitacoradepartamental_2,
  ...
  frm_cuadre, frm_CuadreXCategoria , frm_ReporteDiario_Barco,
  {frm_Opciones_materiales,} frm_NotasGenerales, frm_tripulacion_consumos,
  frm_cuadre_normal, frm_AjustesDiarios;
...
begin
  ...
  begin
    ...
    begin
      ...
      // Sentencia modificada por E. Rangel - 19-08-2016
      QrRecursos.SQL.Add('SELECT bp.sIdPersonal, bp.sDescripcion, (IFNULL(SUM(CAST(bp.dAjuste AS CHAR)), 0) + ' +
                         'IFNULL((SELECT SUM(CAST(ba.dAjuste AS CHAR)) ' +
                         'FROM bitacoradeajustes ba ' +
                         'WHERE ba.sContrato = bp.sContrato AND ba.sNumeroOrden = bp.sNumeroOrden AND ' +
                         'ba.sIdPartidaAnexo = bp.sIdPersonal AND ba.dFecha = bp.dIdFecha), 0)) AS Ajuste, ' +
                         'IF(SUM(bp.dCanthh) > 0, SUM(ROUND(bp.dCanthh, 2)), SUM(bp.dCantidad)) AS Total ' +
                         'FROM bitacoradepersonal bp ' +
                         'WHERE bp.scontrato = :Orden AND bp.sNumeroOrden = :Folio AND bp.didfecha = :Fecha AND ' +
                         'bp.sidPernocta = :Pernocta AND bp.sidplataforma = :Plataforma ' +
      ...
      begin
        ...
        {$REGION 'CONSULTA CUADRE'}
          ...
          QrRecursos.SQL.Add('SELECT bp.sIdEquipo, bp.sDescripcion, IF(SUM(bp.dCanthh) > 0, ROUND(SUM(bp.dCanthh), 2), SUM(bp.dCantidad)) AS Total, ' +
                             ...
                             'FROM bitacoradeajustes ba ' +
                             'WHERE ba.sContrato = bp.sContrato AND ba.sNumeroOrden = bp.sNumeroOrden AND ' +
                             'ba.sIdPartidaAnexo = bp.sIdEquipo AND ba.dFecha = bp.dIdFecha), 0)) AS Ajuste ' +
                             'FROM bitacoradeequipos bp ' +
                             'WHERE bp.scontrato = :Orden AND bp.snumeroorden = :Folio AND ' +
                             'bp.didfecha = :Fecha AND bp.sidPernocta = :Pernocta ' +
...
procedure TfrmDiarioTurno.btnPersonalEquipoClick(Sender: TObject);
Begin
  If ReporteDiario.RecordCount > 0 Then
  begin
...
procedure TfrmDiarioTurno.btnAlcancesClick(Sender: TObject);
begin
...
procedure TfrmDiarioTurno.cxAjustesClick(Sender: TObject);
begin
  Application.CreateForm(TFrmAjustesDiarios, FrmAjustesDiarios);
  FrmAjustesDiarios.sContrato := ReporteDiario.FieldByName('sNumeroOrden').asstring;
  FrmAjustesDiarios.sNumeroOrden := ReporteDiario.FieldByName('sNumeroOrden').asstring;
  FrmAjustesDiarios.dIdFecha := ReporteDiario.FieldByName('dIdFecha').asDatetime;
  FrmAjustesDiarios.ShowModal;
end;
...
procedure TfrmDiarioTurno.btnEmbarqueClick(Sender: TObject);
begin
    {param_global_contrato := ReporteDiario.FieldValues['sOrden'];
    Application.CreateForm(TfrmOpcionesMateriales, frmOpcionesMateriales);
    frmOpcionesMateriales.Show; }
end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            // Modificacion por E. Rangel - 19-08-2016
            sql.text := 'SELECT ' +
                        '  bp.sIdPersonal, ' +
                        '  bp.sDescripcion, ' +
                        '  (IFNULL(SUM(CAST(bp.dAjuste AS CHAR)), 0) + ' +
        ...
        begin
          ...
          sql.text := 'select c.sIdPernocta, c.sDescripcion, c.sMedida, ' +
                      ...
                      '                        FROM ' +
                      '                          bitacoradeajustes ba ' +
                      '                        WHERE ba.sContrato = :contrato AND ' +
                      '                            ba.sNumeroOrden = bp.sNumeroOrden AND ' +
                      '                            ba.sIdPartidaAnexo = bp.sIdPersonal AND ' +
