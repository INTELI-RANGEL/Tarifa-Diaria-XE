...
uses
  ...
  StdCtrls, Mask, DBCtrls, ComCtrls, Grids, DBGrids, global, frm_connection,
  utilerias, DB, ADODB, Buttons, ExtCtrls, frxClass, frxDBSet,
  ZAbstractRODataset, ZDataset, ZAbstractDataset, Controls, Menus,
  UnitExcepciones, udbgrid, UFunctionsGHH, DBDateTimePicker, rxToolEdit,
  RXDBCtrl, RxMemDS, Newpanel, NxCollection, JvExMask, JvToolEdit, JvBaseEdits;
...
begin
    ...
    zColumnas.ParamByName('contrato').AsString := global_contrato;
    zColumnas.Open;

    qry_Catalogo.Active := False;
    qry_Catalogo.ParamByName('fecha').AsDate      := tdIdFecha.Date;
    qry_Catalogo.ParamByName('contrato').AsString := global_contrato;
    qry_Catalogo.Open;
    ...
    qry_personal.ParamByName('contrato').AsString := global_Contrato;
    qry_personal.Open;
...
begin
    ...
    begin
        grid_ordenes.Columns.Add.Title.Caption  := zColumnas.FieldValues['sNumeroOrden'];
        ...
        grid_personal.Columns.Add.Title.Caption := zColumnas.FieldValues['sHoraInicio'] +' '+zColumnas.FieldValues['sHoraFinal'];
...
begin
            ...
            begin
                ...
                begin
                    if (rxPersonal.FieldValues['sContrato'+IntToStr(indice)] = Qry_personal.FieldValues['sContrato']) and
                       ...
                       (rxPersonal.FieldValues['sNumeroOrden'+IntToStr(indice)] = Qry_personal.FieldValues['sNumeroOrden']) and
                    ...
                    begin
                        ...
                        rxPersonal.FieldValues['dCantidad'+IntToStr(indice)] := rxPersonal.FieldValues['dCantidad'+IntToStr(indice)] + xRound(Qry_personal.FieldValues['Cantidad'], 4);
                        rxPersonal.Post;

                        if rxPersonal.FieldValues['sAgrupaPersonal'] <> 'S/C' then
                        begin
                            dCategoria := dCategoria + xRound(Qry_personal.FieldValues['Cantidad'], 2);
                            dTotalDia  := dTotalDia + xRound(Qry_personal.FieldValues['Cantidad'], 2);
                        end;
                    end;
                    inc(indice);
                end;
            ...
            begin
                ...
                begin
                    if (rxPersonal_PU.FieldValues['sContrato'+IntToStr(indice_pu)] = Qry_personal.FieldValues['sContrato']) and
                       ...
                       (rxPersonal_PU.FieldValues['sNumeroOrden'+IntToStr(indice_pu)] = Qry_personal.FieldValues['sNumeroOrden']) and
                    ...
                    begin
                        rxPersonal_PU.Edit;
                        rxPersonal_PU.FieldValues['dCantidad'+IntToStr(indice_pu)] := Qry_personal.FieldValues['Cantidad'];
    ...
    connection.zCommand.Active := False;
    connection.zCommand.SQL.Clear;
    connection.zCommand.SQL.Add('select sum(b.dCantHH) as dCantidad from bitacoradepersonal_cuadre b where b.sContrato =:Contrato and b.dIdFecha =:fecha and b.sAgrupaPersonal <> "S/C" and b.sNumeroOrden <> "@" and b.sHoraInicio <> "00:00" and b.sHoraFinal <> "00:00" '+
                                'group by b.sContrato, b.sNumeroOrden, b.sIdPernocta, b.sIdPlataforma, b.sHoraInicio, b.sHoraFinal '+
                                'order by b.sContrato, b.sNumeroOrden, b.sIdPernocta, b.sIdPlataforma, b.sHoraInicio, b.sHoraFinal');
    ...
    begin
        rxTotales.Edit;
        rxTotales.FieldValues['dTotal'+IntToStr(indice)] := xRound(connection.zCommand.FieldValues['dCantidad'], 2);
        rxTotales.Post;
        connection.zCommand.Next;
        inc(indice);
    end;
...
begin
    ...
    begin
        Connection.Auxiliar.Active := False;
        Connection.Auxiliar.SQL.Clear;
        Connection.Auxiliar.SQL.Add('select a.sNumeroOrden, b.sHoraInicio, b.sHoraFinal, p.iItemOrden, b.* from bitacoradepersonal b '+
                                    ...
                                    'where b.dIdFecha = :fecha and b.sContrato =:contrato and b.sNumeroOrden <> "@" '+
                                    'order by b.sContrato, b.sNumeroOrden,  b.sIdPernocta, b.sIdPlataforma, b.sHoraInicio, b.iItemOrden');
        Connection.Auxiliar.Params.ParamByName('Fecha').Datatype    := ftDate;
        ...
        begin
            connection.zCommand.Active := False;
            connection.zCommand.SQL.Clear;
            Connection.zcommand.SQL.Add('insert into bitacoradepersonal_cuadre (sContrato, dIdFecha, sIdPersonal, iItemOrden, sTipoObra, sDescripcion, sIdPernocta, sIdPlataforma, sNumeroOrden, sHoraInicio, sHoraFinal, dCantidad, dCantHH, sAgrupaPersonal, lAplicaPernocta) '+
                                        'values (:Contrato, :fecha, :personal, :item, :tipoObra, :descripcion, :pernocta, :plataforma, :orden, :inicio, :final, :Cantidad, :CantidadHH, :agrupa, :cobro)');
            while not Connection.Auxiliar.Eof do
            begin
                //try
                    Connection.zcommand.ParamByName('Contrato').AsString    := Connection.Auxiliar.FieldValues['sContrato'];
                    ...
                    Connection.zcommand.ParamByName('final').AsString       := Connection.Auxiliar.FieldValues['sHoraFinal'];
                    Connection.zcommand.ParamByName('cantidad').AsFloat     := Connection.Auxiliar.FieldValues['dCantidad'];
                    Connection.zcommand.ParamByName('cantidadHH').AsFloat   := Connection.Auxiliar.FieldValues['dCantHH'];
                    Connection.zcommand.ParamByName('agrupa').AsString      := Connection.Auxiliar.FieldValues['sAgrupaPersonal'];
                    Connection.zcommand.ParamByName('cobro').AsString       := Connection.Auxiliar.FieldValues['lAplicaPernocta'];
                    Connection.zcommand.ExecSQL;
                    Connection.Auxiliar.Next;
...
begin
    ...
    Connection.zCommand.Active := False;
    Connection.zCommand.SQL.Clear;
    Connection.zCommand.SQL.Add('select * from bitacoradepersonal where dIdFecha =:fecha group by sContrato');
    Connection.zCommand.Params.ParamByName('Fecha').DataType    := ftDate;
    Connection.zCommand.Params.ParamByName('Fecha').Value       := tdIdFecha.date;
    Connection.zCommand.Open;
          ...
          begin
              ...
              begin
                  try
                    ...
                    Q_Cuadre.SQL.Clear;
                    Q_Cuadre.SQL.Add('insert into bitacoradepersonal (sContrato, dIdFecha, sIdPersonal, iItemOrden, sDescripcion, sIdPernocta, sIdPlataforma, sNumeroOrden, sHoraInicio, sHoraFinal, dCantidad, sAgrupaPersonal) '+
