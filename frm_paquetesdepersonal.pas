...
uses
  ...
  Menus, global, ExtCtrls, ImgList, ComCtrls, RXDBCtrl,
...
begin
if grid_paquetes.DataSource.DataSet.IsEmpty=false then
 if grid_paquetes.DataSource.DataSet.RecordCount>0 then
 begin
...
begin
  tsNumeroPaquete.Color := global_color_salida ;
  QryPersonal.Active := False ;
...
begin
    frmBarra1.btnCancel.Click ;
    tsNumeroPaquete.Color := global_color_entrada
end;
...
procedure TfrmPaquetePersonal.Grid_PaquetesMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  UtGrid.dbGridMouseMoveCoord(x,y);
end;

procedure TfrmPaquetePersonal.Grid_PaquetesMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  UtGrid.DbGridMouseUp(Sender,Button,Shift,X, Y);
end;

procedure TfrmPaquetePersonal.Grid_PaquetesTitleClick(Column: TColumn);
begin
...
procedure TfrmPaquetePersonal.Copy1Click(Sender: TObject);
begin
...
procedure TfrmPaquetePersonal.edDescripcionKeyPress(Sender: TObject;
  var Key: Char);
begin
     if key = #13 then
       tsnumeropaquete.SetFocus ;
