...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, frm_connection, global, ToolWin, ExtCtrls,
  DBCtrls, Mask, frm_barra, db, Menus, OleCtrls, frxClass, frxDBSet, Buttons,
  RxLookup, RxMemDS, utilerias, Newpanel, RXCtrls, DateUtils, math,
  unitEstimacion, strUtils, ImgList, frxPreview,  frxChart, ZAbstractRODataset,
  ZDataset, Grids, DBGrids, rxToolEdit, RXDBCtrl, CheckLst, JvExControls,
  JvxCheckListBox, ZAbstractDataset, DBDateTimePicker, NxPageControl,
  JvExComCtrls, JvComCtrls, JvExDBGrids, JvDBGrid, JvDBUltimGrid,
  JvMemoryDataset, cxGraphics, cxControls, cxLookAndFeels,
  ...
  cxGridBandedTableView, cxGridDBBandedTableView, cxMemo, AdvTabSet, DBClient,
  AdvProgr;
...
type
  TfrmEstimacionOrdenes = class(TForm)
    ...
    Panel4: TPanel;
    jMryRecursos: TJvMemoryData;
    dsRecursos: TDataSource;
    cxStyleRepository1: TcxStyleRepository;
    cxStyle1: TcxStyle;
    cxStyle2: TcxStyle;
    cxStyle3: TcxStyle;
    ATsRecursos: TAdvTabSet;
    cxGrid1: TcxGrid;
    DbBTDatos: TcxGridDBBandedTableView;
    DbBTDatosColumn1: TcxGridDBBandedColumn;
    DbBTDatosColumn2: TcxGridDBBandedColumn;
    DbBTDatosColumn3: TcxGridDBBandedColumn;
    DbBTDatosColumn4: TcxGridDBBandedColumn;
    cxGrid1Level1: TcxGridLevel;
    ClientDataSet1: TClientDataSet;
    AvProgressAvance: TAdvProgress;
    Panel5: TPanel;
    btnAceptar: TButton;
    CmdCancel: TButton;
    Panel6: TPanel;
    ...
    procedure jMryRecursosFilterRecord(DataSet: TDataSet; var Accept: Boolean);
    procedure FormCreate(Sender: TObject);
    procedure ATsRecursosChange(Sender: TObject; NewTab: Integer;
      var AllowChange: Boolean);
    procedure btnAceptarClick(Sender: TObject);
    procedure NxPCEstimacionChange(Sender: TObject);

  private
    { Private declarations }
    TabChange:Integer;
    IsOpen:Boolean;
    Procedure MyCantValidate(Sender: TField);
    Procedure MyCantGetText(Sender: TField; var Text: string; DisplayText: Boolean);
  public
    { Public declarations }
  end;
...
Procedure TfrmEstimacionOrdenes.MyCantGetText(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  text:=sender.Value;
...
Procedure TfrmEstimacionOrdenes.MyCantValidate(Sender: TField);
var
  tpEvent:TFieldNotifyEvent;
begin
  if Sender.NewValue>jMryRecursos.FieldByName('Limit'+sender.FieldName).AsFloat then
  begin
    raise Exception.Create('No Puede Capturar mas de lo Generado en el dia. ');
    {MessageDlg('No Puede Capturar mas de lo Generado en el dia. ' +
    jMryRecursos.FieldByName(sender.FieldName).AsString, MtError,[MbOk],0);
  ...
  ///   Aqui Hay qye validar las cantidades

end;

procedure TfrmEstimacionOrdenes.NxPCEstimacionChange(Sender: TObject);
begin
  case  NxPCEstimacion.ActivePageIndex of
    0:begin
        self.width:=660;
        self.Height:=478;
      end;
...
procedure TfrmEstimacionOrdenes.FormCreate(Sender: TObject);
begin
  IsOpen:=False;
  NxPCEstimacion.ActivePageIndex:=0;
  NxPCEstimacion.ShowTabs:=False;
end;
...
procedure TfrmEstimacionOrdenes.jMryRecursosFilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
var
  sFTipo:String;
begin
  sFTipo:=Uppercase(ATsRecursos.AdvTabs.Items[TabChange].Caption);
  Accept:=(DataSet.FieldByName('sTipo').AsString=sfTipo);
end;
...
procedure TfrmEstimacionOrdenes.ATsRecursosChange(Sender: TObject;
  NewTab: Integer; var AllowChange: Boolean);
begin
  TabChange:=NewTab;
  jMryRecursos.Filtered:=false;
  jMryRecursos.Filtered:=true;
end;
...
procedure TfrmEstimacionOrdenes.btnAceptarClick(Sender: TObject);
var
  i,TotalCol,RestAv,x,TmpAv:Integer;
  TmpDate:TDate;
  sMes:String;
  BandaPadre:TcxGridBand;
  DbBTDatosColumn : TcxGridDBBandedColumn;
begin
  AvProgressAvance.Position:=0;
  AvProgressAvance.Max:=100;
  AvProgressAvance.Visible:=true;

  TdGenerador(tsOrdenes.KeyValue,tFecha_I.Date,tFecha_F.Date,jMryRecursos,true,AvProgressAvance);
  RestAv:=(100-AvProgressAvance.Position);

  while DbBTDatos.Bands.Count>2 do
  begin
    DbBTDatos.Bands.Items[2].Destroy;
  end;
  TmpDate:=tFecha_I.Date;
  sMes:='';
  i:=0;
  x:=1;
  TmpAv:=0;
  TotalCol:=DaysBetween(tFecha_F.Date,tFecha_I.Date) + 1;
  while TmpDate <= tFecha_F.Date do
  begin
    if sMes <> FormatDateTime('mmmm',TmpDate) then
    begin
      sMes:= FormatDateTime('mmmm',TmpDate);
      Inc(i);

      BandaPadre:=DbBTDatos.Bands.Add;
      with BandaPadre do
      begin
        Caption:=Uppercase(sMes);
        width:=DaysInMonth(TmpDate)* 50;
      end;
    end;

    DbBTDatos.CreateColumn;
    DbBTDatos.Columns[DbBTDatos.ColumnCount-1].Caption:=IntToStr(DayOfTheMonth( TmpDate ));
    DbBTDatos.Columns[DbBTDatos.ColumnCount-1].DataBinding.FieldName :=FormatDateTime('ddddd',TmpDate);
    DbBTDatos.Columns[DbBTDatos.ColumnCount-1].HeaderAlignmentHorz := taCenter;
    DbBTDatos.Columns[DbBTDatos.ColumnCount-1].Width := 50;
    DbBTDatos.Columns[DbBTDatos.ColumnCount-1].Position.BandIndex := BandaPadre.Index;

    jMryRecursos.FieldByName(FormatDateTime('ddddd',TmpDate)).OnValidate:=MyCantValidate;
    jMryRecursos.FieldByName(FormatDateTime('ddddd',TmpDate)).OnGetText:=MyCantGetText;
    TmpDate:=IncDay(TmpDate);

    if TmpAv<>Trunc((x*RestAv)/TotalCol) then
    begin
      AvProgressAvance.Position:=AvProgressAvance.Position +
      (Trunc((x*RestAv)/TotalCol) - TmpAv) ;
      Application.ProcessMessages;
      TmpAv:= Trunc((x*RestAv)/TotalCol);
    end;
    Inc(x);
  end;

  TabChange:=0;
  ATsRecursos.TabIndex:=0;

  jMryRecursos.Filtered:=false;
  jMryRecursos.Filtered:=true;
  AvProgressAvance.Visible:=false;
  NxPCEstimacion.ActivePageIndex:=1;
