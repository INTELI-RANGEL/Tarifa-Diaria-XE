...
uses
  ...
  Mask, Grids, DBGrids, global, Buttons, frxClass, frxDBSet, 
  ...
  UFunctionsGHH,UnitTBotonesPermisos;
...
var
  ...
  //sd;asdssdsssdsassaassasasasaasassa
implementation
...
procedure TfrmConsultaActividad5.ComentariosAdicionalesClick(
  Sender: TObject);
begin
...
begin
 if grid_resumen.DataSource.DataSet.RecordCount>0 then
  begin
    ...
    showmessage('no existen registros para aplicar comentarios');
end
    else
    showmessage('no existen registros para aplicar comentarios');
...
procedure TfrmConsultaActividad5.imgNotasDblClick(Sender: TObject);
begin
...
begin
  tdponderadoanexo.Color:=global_color_entrada;
end;

procedure TfrmConsultaActividad5.tdPonderadoAnexoExit(Sender: TObject);
begin
tdponderadoanexo.Color:= global_color_salida;
end;
...
begin
  tdpreciomn.Color:= global_color_entrada;
end;

procedure TfrmConsultaActividad5.tdPrecioMNExit(Sender: TObject);
begin
  tdpreciomn.Color:= global_color_salida;
end;
...
begin
 tsmedidaanexo.Color:= global_color_entrada;
end;

procedure TfrmConsultaActividad5.tsMedidaAnexoExit(Sender: TObject);
begin
  tsmedidaanexo.Color:= global_color_salida;
end;
...
procedure TfrmConsultaActividad5.mnFichaTecnicaClick(Sender: TObject);
begin
  try
    if grid_resumen.DataSource.Dataset.isempty=false then
    begin
     if grid_resumen.DataSource.DataSet.RecordCount>0 then
      begin
        ...
        //<ROJAS>
            procFichaTecnica (global_contrato, global_convenio, tsNumeroActividad.Text , frmConsultaActividad5,connection.configuracion.fieldbyname('sFormatos').AsString,PermisosExportar(connection.zConnection,global_grupo,sMenuP)) ;
        //
      end
        else
    ...
    on e : exception do begin
      UnitExcepciones.manejarExcep(E.Message, E.ClassName, 'frm_ConsultadeActividades5', 'Al imprimir ficha tecnica', 0);
    end;
  end;
end;
...
begin
    ...
    Begin
        ...
        Connection.qryBusca.SQL.Add('Select Sum(p.dCantidad) as dCantidad From anexo_prequisicion p ' +
                                         'INNER JOIN anexo_requisicion a ON (a.sContrato = p.sContrato And a.iFolioRequisicion = p.iFolioRequisicion) ' +
...
begin
  try
    ...
    Connection.qryBusca2.SQL.Add('Select sNumeroActividad, mDescripcion, sMedida, dCantidadAnexo, dVentaMN, dVentaDLL, sIdFase From actividadesxanexo ' +
                                  'Where sContrato = :Contrato And sIdConvenio = :Convenio And (sMedida<>"PAQUETE" And dCantidadAnexo<>100) ' +
