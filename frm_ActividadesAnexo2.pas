...
uses
  ...
  ExtCtrls, frxClass, frxDBSet, RXDBCtrl, DateUtils,
  math, Newpanel, Sockets, RxLookup, ZAbstractRODataset, unitvalidacion,
  ZAbstractDataset, ZDataset, rxCurrEdit, rxToolEdit, MasUtilerias,
  AdvPanel,
  AdvToolBtn, DBGrids, UnitTBotonesPermisos,
  RxMemDS, ComObj, OleServer, ExcelXP, unitexcepciones, udbgrid, AdvGlowButton,
  unitactivapop, NxPageControl, 
  DBDateTimePicker, UFunctionsGHH, NxCollection;
...
type
  TfrmActividadesAnexo2 = class(TForm)
    ...
    ImprimirExplosiondeInsumos1: TMenuItem;
...
begin
  ...
  roqAnexo.Params.ParamByName('Anexo').DataType    := ftString;
  if pgAnexo.ActivePageIndex = 0 then
     roqAnexo.Params.ParamByName('Anexo').Value    := 'ADM'
  else
     roqAnexo.Params.ParamByName('Anexo').Value    := 'PU';
  ...
  if pgAnexo.ActivePageIndex = 0 then
     ...
     ActividadesxAnexo.Params.ParamByName('Anexo').Value    := 'PU';
  if connection.configuracion.FieldByName('lOrdenaItem').AsString = 'No' then
  begin
...
begin
     tdFechaInicioSub.Color := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tdFechaInicioSubExit(Sender: TObject);
begin
     tdFechaInicioSub.Color := global_color_salida;
end;
...
begin
    tdDuracionSub.Color := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tdDuracionSubExit(Sender: TObject);
begin
    tdDuracionSub.Color := global_color_salida;
end;
...
begin
     tdFechaFinalSub.Color := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tdFechaFinalSubExit(Sender: TObject);
begin
    tdFechaFinalSub.Color := global_color_salida;
end;
...
begin
     tdPonderadoSub.Color := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tdPonderadoSubExit(Sender: TObject);
begin
     tdPonderadoSub.Color := global_color_salida;
end;
...
begin
  ...
  if pgAnexo.ActivePageIndex = 0 then
    ...
    ActividadesxAnexo.FieldValues['sTipoAnexo'] := 'PU';
...
begin
    ...
    if lEdito then
      Kardex('Conceptos Generales', 'Edita  Registro Anexo', tsNumeroActividad.Text, ActividadesxAnexo.FieldValues['sTipoActividad'], '', '', '','Tarifa Diaria','Catalogo de Materiales')
    else
      Kardex('Conceptos Generales', 'Crea   Registro Anexo', tsNumeroActividad.Text, ActividadesxAnexo.FieldValues['sTipoActividad'], '', '', '','Tarifa Diaria','Catalogo de Materiales');
...
begin
  ...
  if pgAnexo.ActivePageIndex = 1 then
  begin
     Grid_Actividades_pu.Enabled := True;
     Grid_Actividades_pu.SetFocus;
  end
  else
  begin
     Grid_Actividades.Enabled := True;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        Kardex('Conceptos Generales', 'Borra Registro Anexo', Actividad, TipoAct, '', '', '','Tarifa Diaria','Anexo C');
...
begin
    tsMedidaSub.color  := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tsMedidaSubExit(Sender: TObject);
begin
    tsMedidaSub.color  := global_color_salida;
end;
...
begin
    tdCantidadSub.Color := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tdCantidadSubExit(Sender: TObject);
begin
    tdCantidadSub.Color := global_color_salida;
end;
...
begin
    tmDescripcionSub.color  := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tmDescripcionSubExit(Sender: TObject);
begin
    tmDescripcionSub.color  := global_color_salida;
end;
...
begin
    tdVentaMNSub.Color := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tdVentaMNSubExit(Sender: TObject);
begin
    tdVentaMNSub.Color := global_color_salida;
end;
...
begin
    tdVentaDLLSub.Color := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tdVentaDLLSubExit(Sender: TObject);
begin
    tdVentaDLLSub.Color := global_color_salida;
end;

procedure TfrmActividadesAnexo2.ActividadesxAnexoAfterScroll(
  DataSet: TDataSet);
begin
  // Sincronizar el datasource con su espejo
...
begin
    tsSubactividad.color  := global_color_entrada;
end;

procedure TfrmActividadesAnexo2.tsSubActividadExit(Sender: TObject);
begin
    tsSubactividad.color  := global_color_salida;
end;
...
begin
  PartidasxAlcance.Refresh;
  frxDetalleAlcance.LoadFromFile(global_files + 'alcancesxpartida.fr3');
...
begin
  AnexoC.LoadFromFile(global_files + 'AnexoC.fr3');
...
procedure TfrmActividadesAnexo2.ImprimirFichaTecnicaxPartida1Click(Sender: TObject);
var
  x, i: integer;
  linea: string;
...
procedure TfrmActividadesAnexo2.ImprimirFichaTecnicaxPartida2Click(Sender: TObject);
var
  x, i: integer;
  linea: string;
  descripcion, sentencia, busca_paquete, muestra_paquete: string;
...
procedure TfrmActividadesAnexo2.GraficadeAnalisisdePU1Click(Sender: TObject);
begin
...
procedure TfrmActividadesAnexo2.BtnBasicosClick(Sender: TObject);
begin
  frmbarra1.btnCancel.Click;
  btnCancel.Click;
...
procedure TfrmActividadesAnexo2.btnPaqueteBasicoClick(Sender: TObject);
begin
...
procedure TfrmActividadesAnexo2.btnPaqueteHerramientaClick(Sender: TObject);
begin
...
procedure TfrmActividadesAnexo2.btnPaquetePersonalClick(Sender: TObject);
begin
...
//////////////////////////////////// PLANTILAS DE IMPORTACION //////////////////
  ...
  // Realizar los ajustes visuales y de formato de hoja
      ...
      Excel.Columns['G:P'].ColumnWidth := 12;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Hoja.Range['A' + IntToStr(Ren) + ':P' + IntToStr(Ren)].Select;
...
//  begin
      ...
      Connection.QryBusca.Filtered := False;
      connection.QryBusca.SQL.Clear;
      connection.QryBusca.SQL.Add('select l.* from alcancesxactividad l ' +
        'inner join actividadesxanexo a on (a.sContrato = l.sContrato and a.sAnexo = l.sAnexo and a.sNumeroActividad = l.sNumeroActividad and a.sWbs=l.sWbs and a.sTipoActividad = "Actividad" ) ' +
...
procedure TfrmActividadesAnexo2.CalculoPU(dParamWbs, dParamActividad: string);
var
  x: integer;
...
begin
  ...
  ImprimirExplosiondeInsumos1.Enabled := pOpcion;
...
begin
  ...
  begin
    ...
    connection.QryBusca.SQL.Clear;
    Connection.QryBusca.Filtered := False;
    connection.QryBusca.SQL.Add('Select p.dVentaMN, p.dVentaDLL, p.sMedida, pq.' + {IdRecurso} TablaRecurso[iParamId][4] + ', pq.dCantidad, pq.sNumeroPaquete ' +
