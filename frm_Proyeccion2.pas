...
uses
  ...
  frxDBSet, RxMemDS, ZAbstractRODataset, ZDataset, rxToolEdit,
...
procedure TfrmProyeccion2.rxAnexoEnter(Sender: TObject);
begin  {
...
procedure TfrmProyeccion2.rxAnexoMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
if (rxanexo.DataSource.DataSet.IsEmpty) or (rxanexo.DataSource.DataSet.RecordCount<=0) then
exit;
  UtGrid.dbGridMouseMoveCoord(x,y);
end;

procedure TfrmProyeccion2.rxAnexoMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
if (rxanexo.DataSource.DataSet.IsEmpty) or (rxanexo.DataSource.DataSet.RecordCount<=0) then
exit;
...
procedure TfrmProyeccion2.rxAnexoTitleClick(Column: TColumn);
begin
if (rxanexo.DataSource.DataSet.IsEmpty) or (rxanexo.DataSource.DataSet.RecordCount<=0) then
exit;
    ...
    case PgProyeccion.ActivePageIndex of
      ...
      begin
        ...
        begin
          ...
          Begin
            frxProyeccion.LoadFromFile (global_files + 'proyectado.fr3') ;
          ...
          Else
          Begin
            frxProyeccion.LoadFromFile (global_files + 'dtGeneradoxMes.fr3') ;
            frxProyeccion.ShowReport;    //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP));
          end ;
        end
        else
      ...
      begin
        ...
        begin
          frxProyeccion.LoadFromFile (global_files + 'ProyeccionPersonal.fr3') ;
          frxProyeccion.ShowReport;    //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP));
        end
        else
        begin
      ...
      begin
        ...
        begin
          frxProyeccion.LoadFromFile (global_files + 'ProyeccionEquipo.fr3') ;
          frxProyeccion.ShowReport;    //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP));
        end
        else
        begin
      ...
      begin
        ...
        begin
          frxProyeccion.LoadFromFile (global_files + 'ProyeccionMaterial.fr3') ;
          frxProyeccion.ShowReport;    //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP));
        end
        else
        begin
...
procedure TfrmProyeccion2.GridEquiposMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
if (GridEquipos.DataSource.DataSet.IsEmpty) or (GridEquipos.DataSource.DataSet.RecordCount<=0) then
exit;
...
procedure TfrmProyeccion2.GridPersonalMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
if (Gridpersonal.DataSource.DataSet.IsEmpty) or (Gridpersonal.DataSource.DataSet.RecordCount<=0) then
exit;
