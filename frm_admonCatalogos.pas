...
uses
  ...
  Grids, DBGrids, JvExDBGrids,UnitTBotonesPermisos, frm_ActividadesAnexo2,
  JvDBGrid, JvDBUltimGrid, NxCustomGridControl,  RXDBCtrl,
  NxDBGrid, NxColumns, StdCtrls, JvExStdCtrls, JvRichEdit, UnitExcepciones,
...
type
  TfrmAdmonCatalogos = class(TForm)
    ...
    anexossTitulo: TStringField;
    JDbCmbCeros: TJvDBComboBox;
    QryTiposdeMovimientolImprimeceros: TStringField;
    ...
    procedure ActualizaCategorias_Tipos;
    procedure UpdateCategorias_Tipos;
    procedure DeleteCategorias_Tipos;
    procedure TiposdePersonalAfterPost(DataSet: TDataSet);
    procedure CategoriasAfterInsert(DataSet: TDataSet);
    procedure TiposdePersonalAfterEdit(DataSet: TDataSet);
    procedure TiposdePersonalAfterInsert(DataSet: TDataSet);
    procedure TiposdePersonalBeforePost(DataSet: TDataSet);
...
begin
  ...
  try
    ...
    //Funcion para dar de alta tiposdepersonal o categorias.. segun el caso..
    ActualizaCategorias_Tipos;
    Categorias.Refresh;
...
begin
  qryTiposdeMovimiento.FieldValues['sClasificacion'] := 'Movimiento de Barco' ;
  qryTiposdeMovimiento.FieldValues['lGrafica'] := 'No' ;
  qryTiposdeMovimiento.FieldValues['lImprimeCeros'] := 'No' ;
  qryTiposdeMovimiento.FieldValues['iColor'] := 5 ;
  qryTiposdeMovimiento.FieldValues['dVentaMN'] := 0.00 ;
  qryTiposdeMovimiento.FieldValues['dVentaDLL'] := 0.00 ;
  qryTiposdeMovimiento.FieldValues['sContrato'] := global_contrato_barco ;
...
begin
    ...
    begin
       ...
       Begin
            ...
            begin
                messageDLG(' No se encontro Embarcacion Principal con el Id ' + QryBarcoVigencia.FieldValues['sIdEmbarcacion']+#10+'Tome en cuenta que las embarcaciones deben darse de alta '+#10+'desde el contrato de barco.', mtInformation, [mbOk], 0);
...
procedure TfrmAdmonCatalogos.TiposdePersonalAfterEdit(DataSet: TDataSet);
begin
    ...
    if connection.zCommand.RecordCount > 0 then
       OldFecha :=  connection.zCommand.FieldByName('dFechaVigencia').AsDateTime;

    OldCategoria   := TiposdePersonal.FieldValues['sIdTipoPersonal'];
    sOpcion        := 'edit';
end;

procedure TfrmAdmonCatalogos.TiposdePersonalAfterInsert(DataSet: TDataSet);
begin
    sOpcion := 'insert'
end;

procedure TfrmAdmonCatalogos.TiposdePersonalAfterPost(DataSet: TDataSet);
begin
   ...
   begin
        NewCategoria   := TiposdePersonal.FieldByName('sIdTipoPersonal').AsString;
        NewDescripcion := TiposdePersonal.FieldByName('sDescripcion').AsString;
        UpdateCategorias_Tipos;
   end;
   Categorias.Refresh;
end;

procedure TfrmAdmonCatalogos.TiposdePersonalBeforePost(DataSet: TDataSet);
var
...
begin
    ...
    if OldCategoria <> TiposdePersonal.FieldValues['sIdTipoPersonal'] then
    begin
        if (OldCategoria = 'PE-C') or (OldCategoria = 'PE-S') or (OldCategoria = 'PE-T') or (OldCategoria = 'PU')  then
        begin
            messageDLG('Id Tipo Personal restringido por el Sistema!', mtWarning, [mbOk], 0);
            TiposdePersonal.Cancel;
            exit;
        end;

        BuscaCat.Active := False;
        BuscaCat.SQL.Clear;
        BuscaCat.SQL.Add('Select sIdTipoPersonal from tiposdepersonal where sIdTipoPersonal =:Tipo');
        BuscaCat.ParamByName('Tipo').AsString := TiposdePersonal.FieldValues['sIdTipoPersonal'];
        BuscaCat.Open;

        if BuscaCat.RecordCount > 0 then
        begin
            messageDLG('El Id de Tipo de Personal Ya Existe, Favor de Verificar!', mtInformation, [mbOk], 0);
            if TiposdePersonal.State = dsInsert then
               TiposdePersonal.Delete
            else
               TiposdePersonal.Cancel;
        end;
    end
...
begin
  anexos.FieldByName('dventadll').AsFloat:=0;
  anexos.FieldByName('dventamn').AsFloat:=0;
  anexos.FieldByName('sTitulo').asstring:='';
end;
...
procedure TfrmAdmonCatalogos.CategoriasAfterInsert(DataSet: TDataSet);
begin
    sOpcion := 'insert';
    connection.zCommand.Active := False;
    connection.zCommand.SQL.Clear;
    ...
    if connection.zCommand.RecordCount > 0 then
       categorias.FieldValues['dFechaVigencia'] :=  connection.zCommand.FieldByName('dFechaVigencia').AsDateTime;
end;
...
begin
    ...
    if sOpcion = 'edit' then
    begin
        NewCategoria   := Categorias.FieldByName('sIdCategoria').AsString;
        NewDescripcion := Categorias.FieldByName('sDescripcion').AsString;
        UpdateCategorias_Tipos;
    end;
    tiposdepersonal.Refresh;
end;
...
begin
    ...
    begin
        if (OldCategoria = 'PE-C') or (OldCategoria = 'PE-S') or (OldCategoria = 'PE-T') or (OldCategoria = 'PU')  then
        begin
            messageDLG('Id Categoria restringido por el Sistema!', mtWarning, [mbOk], 0);
            Categorias.Cancel;
            exit;
        end;
...
procedure TfrmAdmonCatalogos.ActualizaCategorias_Tipos;
var
   dFechaVigencia : tDate;
begin
    //Actualizamos los tipos de personal
    connection.zCommand.Active := False;
    connection.zCommand.SQL.Clear;
    connection.zCommand.SQL.Add('select sIdCategoria,sDescripcion,dFechaVigencia from categorias where dFechaVigencia <=:fecha ');
    connection.zCommand.ParamByName('fecha').AsDate   := Date;
    connection.zCommand.Open;
    ...
    begin
        connection.QryBusca.Active := False;
        connection.QryBusca.SQL.Clear;
        connection.QryBusca.SQL.Add('select sIdTipoPersonal from tiposdepersonal where sIdTipoPersonal =:tipo ');
        connection.QryBusca.ParamByName('tipo').AsString := connection.zCommand.FieldByName('sIdCategoria').AsString;
        connection.QryBusca.Open;

        if connection.QryBusca.RecordCount = 0 then
        begin
            //Sino se encuentra insertamos el tipo de personal.
            connection.QryBusca.Active := False;
            connection.QryBusca.SQL.Clear;
            connection.QryBusca.SQL.Add('insert into tiposdepersonal (sIdTipoPersonal, sDescripcion, sMascara, lPersonalEQ, lImprimeConcentrado, lPernocta) '+
                                        'values (:tipo, :descripcion, :mascara, "No", "No", "No")');
            connection.QryBusca.ParamByName('tipo').AsString        := connection.zCommand.FieldByName('sIdCategoria').AsString;
            connection.QryBusca.ParamByName('descripcion').AsString := connection.zCommand.FieldByName('sDescripcion').AsString;
            connection.QryBusca.ParamByName('mascara').AsString     := copy(connection.zCommand.FieldByName('sDescripcion').AsString, 0, 4) + '-';
            connection.QryBusca.ExecSQL;
        end;
        connection.zCommand.Next;
    end;

    //Ahora las categorias en caso de que no existan en los tipos..
    connection.zCommand.Active := False;
    connection.zCommand.SQL.Clear;
    connection.zCommand.SQL.Add('select sIdTipoPersonal,sDescripcion from tiposdepersonal ');
    connection.zCommand.Open;
    ...
    begin
        connection.QryBusca.Active := False;
        connection.QryBusca.SQL.Clear;
        connection.QryBusca.SQL.Add('select sIdCategoria from categorias where dFechaVigencia =:fecha and sIdCategoria =:categoria ');
        connection.QryBusca.ParamByName('categoria').AsString := connection.zCommand.FieldByName('sIdTipoPersonal').AsString;
        connection.QryBusca.ParamByName('fecha').AsDate       := dFechaVigencia;
        connection.QryBusca.Open;
        ...
        begin
            ...
            connection.QryBusca.Active := False;
            connection.QryBusca.SQL.Clear;
            connection.QryBusca.SQL.Add('insert into categorias (sIdCategoria, dFechaVigencia, sDescripcion, sMiGrupoResumen, lPersonalAnexo) '+
...
procedure TfrmAdmonCatalogos.UpdateCategorias_Tipos;
var
   dFechaVigencia : tDate;
...
begin
    connection.QryBusca.Active := False;
    connection.QryBusca.SQL.Clear;
    connection.QryBusca.SQL.Add('Update tiposdepersonal set sIdTipoPersonal = :Tipo, sDescripcion =:Descripcion where sIdTipoPersonal =:TipoOld ');
    connection.QryBusca.ParamByName('Tipo').AsString        := NewCategoria;
    connection.QryBusca.ParamByName('TipoOld').AsString     := OldCategoria;
    connection.QryBusca.ParamByName('Descripcion').AsString := NewDescripcion;
    connection.QryBusca.ExecSQL;

    connection.QryBusca.Active := False;
    connection.QryBusca.SQL.Clear;
    connection.QryBusca.SQL.Add('Update categorias set sIDCategoria =:categoria, sDescripcion =:Descripcion where dFechaVigencia =:fecha and sIdCategoria =:CategoriaOld ');
    connection.QryBusca.ParamByName('Categoria').AsString    := NewCategoria;
    connection.QryBusca.ParamByName('CategoriaOld').AsString := OldCategoria;
    connection.QryBusca.ParamByName('Descripcion').AsString  := NewDescripcion;
    connection.QryBusca.ParamByName('Fecha').AsDate          := OldFecha;
    connection.QryBusca.ExecSQL;

    if (NewCategoria <> OldCategoria) then
    begin
        ...
        Actualiza.SQL.Clear;
        Actualiza.SQL.Add('Update tripulacion set sIdCategoria =:Categoria where sContrato =:Contrato and dFechaVigencia =:OldFecha and sIdCategoria =:OldCategoria');
        Actualiza.ParamByName('Contrato').AsString     := global_contrato_barco;
        ...
        Actualiza.Active := False;
        Actualiza.SQL.Clear;
        Actualiza.SQL.Add('Update personal set sIdTipoPersonal =:Categoria where sIdTipoPersonal =:OldCategoria');
        Actualiza.ParamByName('OldCategoria').AsString := OldCategoria;
        Actualiza.ParamByName('Categoria').AsString    := Categorias.FieldValues['sIdCategoria'];
        Actualiza.ExecSQL;

        messageDLG('Se han Actualizado las tablas dependientes por el Cambio de Id para: '+#10+ '['+NewDescripcion+']', mtInformation, [mbOk], 0);
        Actualiza.Destroy;
    end;
end;

procedure TfrmAdmonCatalogos.DeleteCategorias_Tipos;
var
    BuscaCat : TZReadOnlyQuery;
    sMeNsaje  : string;
begin
    ...
    BuscaCat.Active := False;
    BuscaCat.SQL.Clear;
    BuscaCat.SQL.Add('Select sIdCategoria from tripulaciondiaria_listado where sIdCategoria =:Categoria limit 1');
    BuscaCat.ParamByName('Categoria').AsString := Categorias.FieldValues['sIdCategoria'];
    BuscaCat.Open;

    if BuscaCat.RecordCount > 0 then
        sMensaje := sMensaje + #10 +' - CATEGORIA ['+Categorias.FieldValues['sIdCategoria']+'] REPORTADA EN LISTA DE PERSONAL.';

    BuscaCat.Active := False;
    BuscaCat.SQL.Clear;
    BuscaCat.SQL.Add('Select sIdTipoPersonal from personal where sIdTipoPersonal =:Categoria limit 1');
    BuscaCat.ParamByName('Categoria').AsString := Categorias.FieldValues['sIdCategoria'];
    BuscaCat.Open;

    if BuscaCat.RecordCount > 0 then
        sMensaje := sMensaje + #10 + ' - CATEGORIA ['+Categorias.FieldValues['sIdCategoria']+'] ASIGNADA A CATALOGO DE PERSONAL.';

    BuscaCat.Active := False;
    BuscaCat.SQL.Clear;
    BuscaCat.SQL.Add('Select sIdCategoria from tripulacion where sIdCategoria =:Categoria limit 1');
    BuscaCat.ParamByName('Categoria').AsString := Categorias.FieldValues['sIdCategoria'];
    BuscaCat.Open;

    if BuscaCat.RecordCount > 0 then
        sMensaje := sMensaje + #10 + ' - CATEGORIA ['+Categorias.FieldValues['sIdCategoria']+'] ASIGNADA A CATALOGO DE TRIPULACION.';

    BuscaCat.Destroy;
    if sMensaje <> '' then
    begin
       messageDLG('No se puede Eliminar!, La Categoria está siendo utilizada de la siguiente forma:' +#10+ sMensaje, mtWarning, [mbOk], 0);
       abort;
    end;
end;
