...
uses
  ...
  Grids, DBGrids, global, Buttons, frxClass, frxDBSet, 
...
try
   ...
   rDiario.LoadFromFile (global_files + 'DetPartidasxPaquete.fr3') ;
   ...
   Else
      ...
      //
   if not FileExists(global_files + 'DetPartidasxPaquete.fr3') then
   showmessage('El archivo de reporte DetPartidasxPaquete.fr3 no existe, notifique al administrador del sistema');
except
 on e : exception do begin
...
if GridActividades.DataSource.DataSet.IsEmpty=true then
 ...
 try
  ...
  End ;
  QryPartidas.Destroy ;
  rDiario.LoadFromFile (global_files + 'DetAvancePartidasxPaquete.fr3') ;
  If connection.configuracion.FieldValues['lLicencia'] = 'No' Then
      rDiario.PreviewOptions.Buttons := [pbZoom,pbFind,pbOutline,pbPageSetup,pbTools,pbNavigator]
  Else
      rDiario.PreviewOptions.Buttons := [pbPrint,pbExport,pbZoom,pbFind,pbOutline,pbPageSetup,pbTools,pbNavigator,pbExportQuick] ;
      ...
      rDiario.ShowReport;   //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP)) ;
      //
   if not FileExists(global_files + 'DetAvancePartidasxPaquete.fr3') then
   showmessage('El archivo de reporte DetAvancePartidasxPaquete.fr3 no existe, notifique al administrador del sistema');
 except
  on e : exception do begin
...
if GridActividades.DataSource.DataSet.IsEmpty=true then
  ...
  try
   ...
   rDiario.LoadFromFile (global_files + 'DetRetrazosPaquete.fr3') ;
   If connection.configuracion.FieldValues['lLicencia'] = 'No' Then
      rDiario.PreviewOptions.Buttons := [pbZoom,pbFind,pbOutline,pbPageSetup,pbTools,pbNavigator]
   Else
      rDiario.PreviewOptions.Buttons := [pbPrint,pbExport,pbZoom,pbFind,pbOutline,pbPageSetup,pbTools,pbNavigator,pbExportQuick] ;
      ...
      rDiario.ShowReport;   //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP));
      //
   if not FileExists(global_files + 'DetRetrazosPaquete.fr3') then
   showmessage('El archivo de reporte DetRetrazosPaquete.fr3 no existe, notifique al administrador del sistema');
  except
   on e : exception do begin
...
if GridActividades.DataSource.DataSet.IsEmpty=true then
 ...
 try
   ...
   dbReporte.FieldAliases.Clear ;
   dbReporte.DataSet := ActividadesIguales ;
   rDiario.LoadFromFile (global_files + 'DetAlcancesPartidasPaquete.fr3') ;
   If connection.configuracion.FieldValues['lLicencia'] = 'No' Then
      rDiario.PreviewOptions.Buttons := [pbZoom,pbFind,pbOutline,pbPageSetup,pbTools,pbNavigator]
   Else
      rDiario.PreviewOptions.Buttons := [pbPrint,pbExport,pbZoom,pbFind,pbOutline,pbPageSetup,pbTools,pbNavigator,pbExportQuick] ;
      ...
      rDiario.ShowReport;   //(connection.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(connection.zConnection, global_grupo, sMenuP));
      //
   if not FileExists(global_files + 'DetAlcancesPartidasPaquete.fr3') then
   showmessage('El archivo de reporte DetAlcancesPartidasPaquete.fr3 no existe, notifique al administrador del sistema');
 except
  on e : exception do begin
