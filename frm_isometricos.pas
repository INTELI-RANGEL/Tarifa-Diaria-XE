...
begin
  ...
  UtGrid:=TicdbGrid.create(grid_isometricos);
...
begin
     ...
     BotonPermiso.permisosBotones(frmBarra1);
     grid_isometricos.Enabled := False;
end;
...
begin
   ...
   frxImpresion.LoadFromFile(Global_Files+'IsometricosGeneral.fr3') ;
...
begin
    tiRevision.Color := global_color_entrada
end;

procedure TfrmIsometricos.tiRevisionExit(Sender: TObject);
begin
    tiRevision.Color := global_color_salida
end;
...
begin
    If PartidasxIsometrico.State = dsBrowse Then
      If PartidasxIsometrico.FieldValues['sDescripcion'] <> '' Then
        grid_partidas.Hint := PartidasxIsometrico.FieldValues['sDescripcion']
end;
