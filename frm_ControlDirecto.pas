...
uses
  ...
  ExtCtrls, Menus, global, DateUtils, StrUtils, frxClass,
  frxDBSet, Utilerias, RxCombos, jpeg, Series, Chart,
  DbChart, Newpanel, Buttons, TeEngine, TeeProcs,
  ZAbstractRODataset, ZDataset, NxColumnClasses, NxColumns,
  NxScrollControl, NxCustomGridControl, NxCustomGrid, NxGrid, masUtilerias, 
  NxPageControl, udbgrid, UFunctionsGHH ;
...
begin
   dbGraphics.SeriesList.Items[1].Active  := chkProgramado.Checked ;
end;
...
begin
  try
      If Generadores.RecordCount > 0 Then
          If lfnValidaGenerador (TreeObras.Selected.Text, cnfGrafica.FieldValues['sIdConvenio'], Generadores.FieldValues['sNumeroOrden'], Generadores.FieldValues['sNumeroGenerador'] , frmControlDirecto ) Then
              procNumeroGenerador (TreeObras.Selected.Text, Generadores.FieldValues['iNumeroEstimacion'] , Generadores.FieldValues['sNumeroOrden'] , Generadores.FieldValues['sNumeroGenerador'] , cnfGrafica.FieldValues['sIdConvenio'], 'Cliente', frmControlDirecto, frGenerador.OnGetValue, True, connection.configuracion.fieldbyname('sFormatos').AsString,PermisosExportar(connection.zConnection,global_grupo,'AccesoObra'))
          Else
              MessageDlg('Existen partidas adicionales de anexo en el generador seleccionado, es necesario adicionar la nota de cambio acerca del motivo por el cual se excedieron las partidas.' , mtWarning, [mbOk], 0) ;
  except
      on e : exception do begin
...
begin
  try
      If Generadores.RecordCount > 0 Then
          If lfnValidaGenerador (TreeObras.Selected.Text, cnfGrafica.FieldValues['sIdConvenio'], Generadores.FieldValues['sNumeroOrden'], Generadores.FieldValues['sNumeroGenerador'] , frmControlDirecto ) Then
              procNumeroGenerador (TreeObras.Selected.Text, Generadores.FieldValues['iNumeroEstimacion'] , Generadores.FieldValues['sNumeroOrden'] , Generadores.FieldValues['sNumeroGenerador'] , cnfGrafica.FieldValues['sIdConvenio'], 'Contratista', frmControlDirecto, frGenerador.OnGetValue, True, connection.configuracion.fieldbyname('sFormatos').AsString,PermisosExportar(connection.zConnection,global_grupo,'AccesoObra'))
          Else
              MessageDlg('Existen partidas adicionales de anexo en el generador seleccionado, es necesario adicionar la nota de cambio acerca del motivo por el cual se excedieron las partidas.' , mtWarning, [mbOk], 0) ;
  except
      on e : exception do begin
...
begin
  try
      If Generadores.RecordCount > 0 Then
          If lfnValidaGenerador (TreeObras.Selected.Text, cnfGrafica.FieldValues['sIdConvenio'], Generadores.FieldValues['sNumeroOrden'], Generadores.FieldValues['sNumeroGenerador'] , frmControlDirecto ) Then
               procSemanalSinConImportes(TreeObras.Selected.Text, Generadores.FieldValues['iNumeroEstimacion'] , Generadores.FieldValues['sNumeroOrden'] , Generadores.FieldValues['sNumeroGenerador'] , cnfGrafica.FieldValues['sIdConvenio'], 'Sin Importes', frmControlDirecto, frGenerador.OnGetValue, True, connection.configuracion.fieldbyname('sFormatos').AsString,PermisosExportar(connection.zConnection,global_grupo,'AccesoObra'))
          Else
              MessageDlg('Existen partidas adicionales de anexo en el generador seleccionado, es necesario adicionar la nota de cambio acerca del motivo por el cual se excedieron las partidas.' , mtWarning, [mbOk], 0) ;
  except
      on e : exception do begin
...
begin
  try
      If Generadores.RecordCount > 0 Then
          If lfnValidaGenerador (TreeObras.Selected.Text, cnfGrafica.FieldValues['sIdConvenio'], Generadores.FieldValues['sNumeroOrden'], Generadores.FieldValues['sNumeroGenerador'] , frmControlDirecto ) Then
               procSemanalSinConImportes(TreeObras.Selected.Text, Generadores.FieldValues['iNumeroEstimacion'] , Generadores.FieldValues['sNumeroOrden'] , Generadores.FieldValues['sNumeroGenerador'] , cnfGrafica.FieldValues['sIdConvenio'], 'Con Importes', frmControlDirecto, frGenerador.OnGetValue, True, connection.configuracion.fieldbyname('sFormatos').AsString,PermisosExportar(connection.zConnection,global_grupo,'AccesoObra'))
          Else
              MessageDlg('Existen partidas adicionales de anexo en el generador seleccionado, es necesario adicionar la nota de cambio acerca del motivo por el cual se excedieron las partidas.' , mtWarning, [mbOk], 0) ;
  except
      on e : exception do begin
...
begin
    try
        If Generadores.RecordCount > 0 Then
            If lfnValidaGenerador (TreeObras.Selected.Text, cnfGrafica.FieldValues['sIdConvenio'], Generadores.FieldValues['sNumeroOrden'], Generadores.FieldValues['sNumeroGenerador'] , frmControlDirecto ) Then
                procListadeVerificacion (TreeObras.Selected.Text, Generadores.FieldValues['iNumeroEstimacion'] , Generadores.FieldValues['sNumeroOrden'] , Generadores.FieldValues['sNumeroGenerador'] , cnfGrafica.FieldValues['sIdConvenio'], frmControlDirecto, frGenerador.OnGetValue, False, connection.configuracion.fieldbyname('sFormatos').AsString, PermisosExportar(connection.zConnection,global_grupo,'AccesoObra'))
            Else
                MessageDlg('Existen partidas adicionales de anexo en el generador seleccionado, es necesario adicionar la nota de cambio acerca del motivo por el cual se excedieron las partidas.' , mtWarning, [mbOk], 0) ;
    except
        on e : exception do begin
...
begin
    ...
    if OptMes.Checked  then
      LineAgrupa := ' ,month(a.dIdFecha)';
       //LineAgrupa := ' ,month(dIdFecha)';

    if OptSemana.Checked  then
      LineAgrupa := ' ,week(a.dIdFecha)';
       //LineAgrupa := ' ,week(dIdFecha)';
    ...
    If TreeObras.Selected.Text <> '' then
       ...
       Begin
            ...
            Begin
               ...
               Connection.qryBusca.Active := False ;
               Connection.qryBusca.SQL.Clear ;
               //Connection.qryBusca.SQL.Add('Select Month(dIdFecha) as dMes, year(dIdFecha) as dAnno, Max(dIdFecha) as dIdFecha, Max(dAvancePonderadoGlobal) as dMensual From avancesglobales Where ' +
               //                            'sContrato = :Contrato And sIdConvenio = :Convenio And sNumeroOrden = :Orden Group By Year(dIdFecha)'+ LineAgrupa ) ;
             ...
             //  Connection.qryBusca.Params.ParamByName('Convenio').Value    := cnfGrafica.Fieldbyname('sIdConvenio').AsString ;
              ...
              { if chkAplicaFechas.Checked  = False Then
               ...
               Begin
                  ...
                  {  If StrToDate(sFecha) >= cnfGrafica.FieldValues['dFechaFinal'] Then
                    ...
                    Else  }
                    begin
                         if (tdIdFecha.Date < Connection.qryBusca.FieldValues['dIdFecha']) and (sCorte = 'No') and (OptMes.Checked = False) and (OptDia.Checked = False) then
                         begin
                             ...
                             begin
                                 ...
                                 Connection.qryBusca2.SQL.Add('Select dAvancePonderadoGlobal as dMensual From avancesglobales Where ' +
                                  //                            'sContrato = :Contrato And sIdConvenio = :Convenio And sNumeroOrden = :Orden and dIdFecha =:Fecha ') ;
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          if (Connection.qryBusca.FieldByName('dIdFecha').AsDateTime = tdIdFecha.Date) then
                          begin
                            rxGraficaProgramado.Post;
                            ControlSalto:=0;
                          end
              ...
              Connection.qryBusca2.params.ParamByName('Contrato').DataType := ftString ;
              Connection.qryBusca2.params.ParamByName('Contrato').Value := global_contrato ;    //Pq Global_contrato???
              //Connection.qryBusca2.params.ParamByName('Convenio').DataType := ftString ;
              //Connection.qryBusca2.params.ParamByName('Convenio').Value := global_convenio ;
              Connection.qryBusca2.params.ParamByName('Orden').DataType := ftString ;
              Connection.qryBusca2.params.ParamByName('Orden').Value := tsNumeroOrden.Text ;  // Aqui ya perdio el valor
              Connection.qryBusca2.params.ParamByName('fecha').DataType := ftDate ;
              ...
              Begin
               ...
               // Else
                ...
                {**Respetando el rango de fechas}
                if (chkAplicaFechas.Checked) and ((Connection.qryBusca2.FieldValues['dIdFecha'] >= tdIdFecha1.Date) and (Connection.qryBusca2.FieldValues['dIdFecha'] <= tdIdFecha.Date) ) then
                    rxGraficaProgramado.Post
                else
                    rxGraficaProgramado.Cancel;
                   ...
                   begin
                      ...
                      begin
                          ...
                          QryAvanceReal.SQL.Add('Select a.sWbs, a.dPonderado, sum(b.dAvance) as dAvance, if(sum(b.dAvance) > 100, a.dPonderado, sum(b.dAvance * (a.dPonderado / 100))) as dAvancePonderado '+
                                            'From actividadesxorden a inner join bitacoradeactividades b on (b.scontrato = a.scontrato and b.snumeroorden = a.snumeroorden and b.swbs = a.swbs and b.didfecha <= :Fecha) '+
                          //                  'Where a.sContrato =:Contrato and a.sIdConvenio =:Convenio And a.sNumeroOrden =:Orden And b.sWbs like concat(:Principal, ".%") group by a.sWbs');
                         ...
                         // QryAvanceReal.ParamByName('Convenio').AsString  := cnfGrafica.FieldValues['sIdConvenio'];
                          ...
                          begin
                              ...
                              begin
                                 // dSumaAvance := dSumaAvance + QryAvanceReal.FieldValues['dAvancePonderado'];
                      ...
                      begin
                         ...
                         // Connection.qryBusca2.Params.ParamByName('Convenio').Value    := cnfGrafica.FieldValues['sIdConvenio'] ;
                          ...
                          Begin
                               rxGraficaFisico.Append ;
                               rxGraficaFisico.FieldValues['sDescripcion'] := TreeObras.Selected.Text ;
                               rxGraficaFisico.FieldValues['dFecha']       := Connection.qryBusca.FieldValues['dIdFecha']  ;
